...
BOOL filopen(HWND hwnd, const char* filter, const char* title, const char* path)
{
  ...
  ofn.lpstrFilter = filter;         //TEXT("EEC Binary(*.bin)\0*.bin\0");
  ofn.nFilterIndex = 1;
  ofn.lpstrInitialDir = path;           //fname[0];                 // set THIS to required path
  ofn.lpstrFileTitle = NULL;
  ofn.lpstrTitle = title;                 //"Select EEC Binary";
        ...
        LPSTR lpCmdLine, int nCmdShow)
{
        ...
        wc.hbrBackground = (HBRUSH)(COLOR_BACKGROUND+1);         //COLOR_WINDOW+1); (COLOR_GRAYTEXT+1);
