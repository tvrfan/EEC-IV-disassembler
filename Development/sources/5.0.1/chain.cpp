...
int pl_cmd(CHAIN *,uint, uint (*prt) (uint,cchar *, ...));
int pl_psw(CHAIN *,uint, uint (*prt) (uint,cchar *, ...));
int pl_sym(CHAIN *,uint, uint (*prt) (uint,cchar *, ...));
int pl_sub(CHAIN *,uint, uint (*prt) (uint,cchar *, ...));
int pl_rbt(CHAIN *,uint, uint (*prt) (uint,cchar *, ...));
int pl_adl(CHAIN *,uint, uint (*prt) (uint,cchar *, ...));
int pl_bnk(CHAIN *,uint, uint (*prt) (uint,cchar *, ...));
int pl_spf(CHAIN *,uint, uint (*prt) (uint,cchar *, ...));
...
//  void *rgstat[0x400];               //  register status (pointers), including rbase inv count.
//  make this a chain, but use direct lookup instead of bfind? makes it compatible with all other code...and links.
...
//a->size +=1; somewhere to get extra pointer for malloc tracking make sure all sizeofs are > void * ??.....
    ...
    // make first data entry a pointer to next block for clean free...
    // resinstate dbgmalloced for tracking.........

    n = NULL;

    if (x->esize)
     {
      msz = (x->esize*x->asize); // + sizeof(void*);         // size of new data block to go with ptrs + extra pointer
      n =  (char *) mem(0,0,msz);
      if (!n) wnprt(0,"FATAL - MALLOC ERROR ");
     }

    z = n; // + sizeof(void*);                    //skip 'base' pointer
...
if (x->chfree)
 ...
 // blocks allocated every x->asize, so can use this to release data, but can't as they have all
 // been shuffled !!!
 ...
 for (i = 0; i < x->num; i += x->asize)
   {
    mfree (x->ptrs[i], x->esize*x->asize);
   } */
...
//wait, what about the data itself ??? was that what mlblk was for ??? YES YES YES ....
...
int cpsym (CHAIN *x, uint ix, void *newb)
{
 ...
 ans = ch->fend - nw->fend;     //includes WRITE
 if (ans) return ans;
...
// math terms, name. use strcmp for name ordering
 ...
 return strcmp(ch->mathname, nw->mathname);
...
ADT *get_last_adt(void *fid)
 {
   // last one in chain
  // as all structs have fid as first item, should work for any chain
  // subchains ???
...
uint chk_modblk(LBK* blk)
{
  ...
  if (blk->start + bytes(c->dfend) > blk->end) ans = 1;
...
void check_chain(uint ch,uint min)
...
//    similar to --- but for adjacent chain blocks....
// if (x->num == 0) return 1;        //ok to insert if chain empty

 // ans = 1;                            // set 'insert OK'

  //get range of possible overlaps from proposed insert

  //min = find_olap(x, 0, inx, x->newins);

  // if (min == inx) return 1;              // no overlaps

  i = min;

  while (i < x->num)
    {
      chkf = olbchk(x, i,i+1,1);              // check if chain block (i) overlaps with next block
      if (!chkf) break;                               // no overlap

      fix_overlap (ch, i, i+1, chkf);
...
LBK* add_cmd(uint start, uint end, uint com, uint from)
 ...
 //only code at 0x2000 in each bank
   ...
   if (cmd->dfend > 7 && (start & 1))       ///  longs must be on long (4)??? move to command ???
    {
...
void *get_last_spf(void *fid)
 {
   // last one in chain
  // as all structs have fid as first item, should work for any chain
  //what about subchains ???
...
/*
void copy_adt_chain(void *dfid, void *sfid)
{
    // dfid <- sfid         dest = sce
    //and subs ?? probably
 ADT *a, *b;

 b = (ADT*) get_last_adt(dfid);       //end of any destination chain

 while ((a = get_adt(sfid,0)))
  {
      //but what about subfields ??? what about math links ????  copy from print ??

    b = add_adt(b,a->sbix);        // probably always zero, but just in case....
    *b = *a;
    b->fid = dfid;       // restore key
    dfid = b;            // next dest
    sfid  = a;           // next sce
  }
...
SPF * copy_spf(void *dfid, SPF * f, int inc)
{
 // copy a single spf to new fid
//how to tell if more than one exists ??

//sort out d->sec flag !!

 SPF *d;

   dfid = get_last_spf(dfid);

   d = add_spf(dfid, 1,1);         // dummy values for command
   *d = *f;                        //copy everything
   d->fid = dfid;                 // restore key
   d->level = f->level+inc;       // set new level
  // d->adtdone = 0;
  // f->adtdone = 1;
 //   sfid  = a;           // next sce
 // }
...
// do we need optional chain refs ??  maybe....
...
/*
  if (ans) *ans = k->nextaddr;

      {
       if (k->type < 3) *ans = k->sc;  else *ans = 0;
      }

   if (psp)
     {
       if (k->type == 3) *psp = k->start; else *psp = 0;
     }
   return k->type;
}
*/
...
void fake_push(SBK *s, uint type, SBK *newblk)
 {
  ...
  // NOT pushp
   ...
   if (k->sceaddr == s->curaddr)       // && k->sc == newblk)
    {
             #ifdef XDBGX
...
/*  if (s->ch == CHSCAN)
     {
    //   s->proctype = 1;                      // std scan processing
       s->logdata  = 1;                        // and log any data found
     }
   else s->logdata = 0;                      // test scanning, and don't log data
*/
   ...
   ix = type & 0x1f;
   if (caller) s->caller = caller;                      // jump from
...
//       s->logdata  = caller->logdata;        // keep nodata flag
       ...
       if (s->ssub)       //ix == J_SUB || ix == J_VECT)        //s->ssub || s->svect)                //calltype == J_SUB || s->calltype == J_VECT)
        {

         if (ix == J_SUB)                        //calltype == J_SUB)
...
uint find_argblk(SBK *s, uint ix, OPS *o)
 ...
 // g->argsblk = 0;
  ...
  scb = get_scan(CHSCAN, tinst.opr[1].addr);       // 'real' scan version
   //  p->tgtblk[0] = get_scan(CHSCAN, tinst.opr[1].addr);       // 'real' scan version
...
p is NULL
       ...
       if (p->tgtblk[0])  DBGPRT(0, " tgt[0] = %x (%x)", p->tgtblk[0]->nextaddr,p->tgtblk[0]->start);
...
*/
 ...
 // scx = get_scan(CHSTST, tinst.opr[1].addr);       // 'emu' scan version (ptarget)
  ...
  if (s->proctype == 4)
    {
      p->tgtblk[1] = get_scan(CHSTST, tinst.opr[1].addr);       // emu version
      if (!p->tgtblk[1])
         {
                  #ifdef XDBGX
           DBGPRT(1,"NO EMU SCAN pointer !! STOP (%x)", p->tgtblk[0]->start);
           #endif

      //possible that no emu block exists, so copy the scan block NO - stop the pop instead
      g->popped = 0;
      return 0;

    //      p->tgtblk[1] = add_escan(0,p->tgtblk[0]);
         }        //g->scanblk);            //copy it to the emu chain
    }

  if (s->proctype == 1 && p->tgtblk[0]) p->argblk[0] = get_scan(CHSCAN,k->start);

 // test = get_scan(CHSCAN,k->start);
 // test1 = get_scan(CHSCAN,k->nextaddr);


  if (s->proctype == 4 && p->tgtblk[1]) p->argblk[1] = get_scan(CHSTST,k->start);
...
uint fake_pop(SBK *s, OPS *o)
{
 ...
 //  RPOP *p;
  ...
  // type = 3   pushp  (PSW)
   ...
   else if (k->type == 1)    //true CALLs only - do argblks
     {
      o->val = k->nextaddr;
      find_argblk(s,0,o);
     }
...
// fend also extra flags, such as SYS and CMD.
 ...
 lfend = (fend & 0x1f);                   // = 31, get size only.
 ...
 if (fend & C_SYS)
  ...
  //        s->xnum = 1;                 // stop overwrites by autoname NOT USED
        }
 ...
 if (fstart > 7)
   ...
   if (fstart == 0)
     {         // 'whole' or default symbol
       if (lfend == 7 || lfend == 15) s->whole = 1;
     }
...
*/
   ...
   if ((t->fend & 0x40) != (s->fend & 0x40)) return;    // read versus write, OK

   if (s->whole != t->whole)  return;           // bit versus default/whole, OK

   // address range checks.

   if (s->rstart < t->rstart && s->rend > t->rend) return;         // spans
   if (s->rstart > t->rstart && s->rend < t->rend) return;         // contained
   if (s->rend   < t->rstart)   return;                            // before
   if (s->rstart > t->rend)     return;                            // after

   // ranges must overlap to get to here

   if (s->whole)
     {
       // whole syms cannot overlap  - what about 0-7 versus 0-15 ?
       chsym.lasterr = E_OVRG;               //range overlap
       return;
     }
 ...
 //  if (s->fstart <= t->fstart && s->fend   >= t->fstart && s->fend   <= t->fend) bl = 1;   // O overlaps field start
 //  if (s->fend   >= t->fend   && s->fstart >= t->fstart && s->fstart <= t->fend) bl = 1;   // O overlaps rear


 //  if (rl & bl)
 chsym.lasterr = E_OVLP; // field overlaps
...
// !ans is an exact duplicate, including addr,fstart,fend, start range, but not end range.
   ...
   if (ix < chsym.num)
    ...
    //  if (s->smask) DBGPRT(0," mask %x", s->smask);
...
SYM ** get_syms(uint add, uint fstart, uint fend, uint pc)
{
  ...
  list = (SYM**) &scores;           // overlay SYM* array onto scores array
  add = fix_bare_addr(add);         //  for bank, not bit

//check fstart - fend has 0x40 as write marker

  end = add + bytes(fend) -1;

  s = (SYM*) chmem(CHSYM,3);     // block for search , a little gap..........

  for (ix = 0; ix < 4; ix++) list[ix] = NULL;      // zero defaults and end marker.
  s->addr   = add;
  s->fend   = 0;                   // first possible entry for default address (read, fend=31)
  s->rstart = pc;
  s->rend   = pc;                   // range required for binary search..........
  ix = bfindix(&chsym, s);

  num = 2;                              //defaults at [0] and [1]
  while (ix < chsym.num && num < 256)
    {
     uint z;
     t = (SYM*) chsym.ptrs[ix];        // temp for debug
     ...
     if (pc >= t->rstart && pc <= t->rend)
       {         // within sym range, put deflt read in [0] , deflt write in [1]

        if (t->whole)
         {         // a 'whole' symbol byte or word, keep longest one declared
           if (t->fend & 0x40) z = 1; else z = 0;
           s = list[z];                             // one or zero,  write/read
           if (!s || t->fend > s->fend) list[z] = t;
         }
        else
         {
           list[num++] = t;              // add to list
           list[num] = 0;                // end marker
         }
       }
     ix++;
    }
...
// NOTE !! the overlap between sfr word and byte does NOT NOT NOT overlap into next word !!
// need definition of this somehow !!!




// uint sfrop   : 1 ;     // sfr, cannot assume word mode overlaps into next byte.
...
SYM* get_sym(uint add, uint fstart, uint fend, uint pc)
{
    ...
    list = (SYM**) &scores;   // overlay SYM* onto scores array

    s = 0;

    get_syms(add, fstart, fend, pc);
    ...
    for (i = 2; i < 256; i++)
     ...
     if (s) return s;        // found perfect match

     // no exact match if it got here, so use default

     if (list[1] && (fend & 64)) return  list[1];      // default write sym
     if (list[0])                return  list[0];      // default sym (read or write)
...
void scan_gaps(void)       //  ABCDEF
{
  ...
  //  ans = 0;
    ...
    // DIRS *get_cmd_def(uint cmd)
      DBGPRT(0,"\n\nGAP %x-%x", start, end);
      DBGPRT(1, "  %s <-> %s", get_cmd_str(s->fcom), get_cmd_str(n->fcom));
    #endif
