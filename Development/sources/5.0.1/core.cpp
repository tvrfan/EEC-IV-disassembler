...
* 8065 only, opcode changes PSW,  number of ops, op sizes (1,2,3 as above),
...
{ 3 ,  0, 0,   1, 2,   0x7,  0xf,  0x4f, shl   ,"shlw"  , "\3 <<= \1;" },                // 1  3 = 2 << 1
{ 3 ,  0, 0,   1, 2,   0x7,  0x7,  0x47, shl   ,"shlb"  , "\3 <<= \1;" },
{ 3 ,  0, 0,   1, 2,   0x7,  0x1f, 0x5f, shl   ,"shldw" , "\23 <<= \1;" },

{ 4 ,  0, 0,   1, 2,   0x7,  0xf,  0x4f, shr   ,"shrw"  , "\3 >>= \1;" },                // 4
{ 4 ,  0, 0,   1, 2,   0x7,  0x2f, 0x6f, shr   ,"asrw"  , "\23 >>= \1;" },
{ 4 ,  0, 0,   1, 2,   0x7,  0x1f, 0x5f, shr   ,"shrdw" , "\23 >>= \1;" },               // 6
{ 4 ,  0, 0,   1, 2,   0x7,  0x3f, 0x7f, shr   ,"asrdw" , "\23 >>= \1;" },
{ 4 ,  0, 0,   1, 2,   0x7,  0x7,  0x47, shr   ,"shrb"  , "\3 >>= \1;" },                // 8
{ 4 ,  0, 0,   1, 2,   0x7,  0x27, 0x67, shr   ,"asrb"  , "\23 >>= \1;" },

{ 5 ,  0, 0,   1, 2,   0x7,  0x7,  0x47, andx  ,"an2b"  , "\4\2 &= \1;"  },              // 10
{ 5 ,  0, 0,   1, 2,   0xf,  0xf,  0x4f, andx  ,"an2w"  , "\4\2 &= \1;" },
{ 5 ,  0, 0,   1, 3,   0x7,  0x7,  0x47, andx  ,"an3b"  , "\4\3 = \2 & \1;"  },
{ 5 ,  0, 0,   1, 3,   0xf,  0xf,  0x4f, andx  ,"an3w"  , "\4\3 = \2 & \1;" },

{ 8 ,  1, 0,   1, 2,   0x7,  0x7,  0x4f, mlx   ,"ml2b"  , "\23 = \22 * \1;" },           // 14
{ 8 ,  0, 0,   1, 2,   0x27, 0x27, 0x6f, mlx   ,"sml2b" , "\23 = \22 * \1;"},
{ 8 ,  1, 0,   1, 2,   0xf,  0xf,  0x5f, mlx   ,"ml2w"  , "\23 = \22 * \1;"  },
{ 8 ,  0, 0,   1, 2,   0x2f, 0x2f, 0x6f, mlx   ,"sml2w" , "\23 = \22 * \1;"},

{ 8 ,  1, 0,   0, 3,   0x7,  0x7,  0x4f, mlx   ,"ml3b"  , "\23 = \22 * \1;"  },          // 18
{ 8 ,  0, 0,   0, 3,   0x27, 0x27, 0x6f, mlx   ,"sml3b" , "\23 = \22 * \1;" },           // 19
{ 8 ,  1, 0,   0, 3,   0xf,  0xf,  0x5f, mlx   ,"ml3w"  , "\23 = \22 * \1;"  },          // 20
{ 8 ,  0, 0,   0, 3,   0x2f, 0x2f, 0x7f, mlx   ,"sml3w" , "\23 = \22 * \1;" },           // 21

{ 6 ,  0, 0,   1, 2,   0x7,  0x7,  0x47, add   ,"ad2b"  , "\3 += \1;" },                 // 22
{ 6 ,  0, 0,   1, 2,   0xf,  0xf,  0x4f, add   ,"ad2w"  , "\3 += \1;" },
{ 6 ,  0, 0,   1, 3,   0x7,  0x7,  0x47, add   ,"ad3b"  , "\3 = \2 + \1;"  },
{ 6 ,  0, 0,   1, 3,   0xf,  0xf,  0x4f, add   ,"ad3w"  , "\3 = \2 + \1;" },

{ 7 ,  0, 0,   1, 2,   0x7,  0x7,  0x47, sbt   ,"sb2b"  , "\3 -= \1;" },                 // 26
{ 7 ,  0, 0,   1, 2,   0xf,  0xf,  0x4f, sbt   ,"sb2w"  , "\3 -= \1;" },
{ 7 ,  0, 0,   1, 3,   0x7,  0x7,  0x47, sbt   ,"sb3b"  , "\3 = \2 - \1;"  },
{ 7 ,  0, 0,   1, 3,   0xf,  0xf,  0x4f, sbt   ,"sb3w"  , "\3 = \2 - \1;" },

{ 9 ,  0, 0,   1, 2,   0x7,  0x7,  0x47, orx   ,"orb"   , "\4\3 |= \1;" },               // 30
{ 9 ,  0, 0,   1, 2,   0xf,  0xf,  0x4f, orx   ,"orw"   , "\4\3 |= \1;" },
{ 9 ,  0, 0,   1, 2,   0x7,  0x7,  0x47, orx   ,"xorb"  , "\4\3 ^= \1;" },
{ 9 ,  0, 0,   1, 2,   0xf,  0xf,  0x4f, orx   ,"xrw"   , "\4\3 ^= \1;" },

{ 10,  0, 0,   1, 2,   0x7,  0x7,  0,    cmp   ,"cmpb"  , "" },                          // 34
{ 10,  0, 0,   1, 2,   0xf,  0xf,  0,    cmp   ,"cmpw"  , "" },

{ 11,  1, 0,   1, 2,   0x7,  0xf,  0x47, dvx   ,"divb"  , "\23 = \22 / \1;" },           // 36
{ 11,  0, 0,   1, 2,   0x27, 0x2f, 0x67, dvx   ,"sdivb" , "\23 = \22 / \1;"},            // 37
{ 11,  1, 0,   1, 2,   0xf,  0x1f, 0x4f, dvx   ,"divw"  , "\23 = \22 / \1;" },           // 38
{ 11,  0, 0,   1, 2,   0x2f, 0x3f, 0x6f, dvx   ,"sdivw" , "\23 = \22 / \1;"},            // 39

{ 12,  0, 0,   0, 2,   0x7,  0x7,  0x47, ldx   ,"ldb"   , "\4\3 = \1;" },                // 40
{ 12,  0, 0,   0, 2,   0xf,  0xf,  0x4f, ldx   ,"ldw"   , "\4\3 = \1;" },

{ 6 ,  0, 0,   1, 2,   0x7,  0x7,  0x47, add   ,"adcb"  , "\3 += \1 + CY;" },            // 42
{ 6 ,  0, 0,   1, 2,   0xf,  0xf,  0x4f, add   ,"adcw"  , "\3 += \1 + CY;" },
{ 7 ,  0, 0,   1, 2,   0x7,  0x7,  0x47, sbt   ,"sbbb"  , "\3 -= \1 - CY;" },
{ 7 ,  0, 0,   1, 2,   0xf,  0xf,  0x4f, sbt   ,"sbbw"  , "\3 -= \1 - CY;" },

{ 12,  0, 0,   0, 2,   0x7,  0x7,  0x4f, ldx   ,"ldzbw" , "\23 = \21;" },                // 46
{ 12,  0, 0,   0, 2,   0x27, 0x7,  0x6f, ldx   ,"ldsbw" , "\23 = \21;" },

{ 13,  0, 0,   0, 2,   0x7,  0x7,  0x47, stx   ,"stb"   , "\4\1 = \2;" },                // 48
{ 13,  0, 0,   0, 2,   0xf,  0xf,  0x4f, stx   ,"stw"   , "\4\1 = \2;" },                 //  N.B. ops swopped later

{ 14,  1, 0,   0, 1,   0xf,  0,    0,    pshw  ,"push"  , "push(\1);" },                 // 50
{ 14,  0, 1,   0, 1,   0xf,  0,    0,    pshw  ,"pusha" , "pusha(\1);" },
{ 15,  1, 0,   0, 1,   0xf,  0,    0x4f, popw  ,"pop"   , "\1 = pop();" },               // 52
{ 15,  0, 1,   0, 1,   0xf,  0,    0x4f, popw  ,"popa"  , "\1 = popa();" },

{ 19,  0, 0,   0, 1,   0,    0,    0,    cjm   ,"jnst"  , "if (SKY = 0) \x9" },          // 54  to 71 cond jumps
{ 19,  0, 0,   0, 1,   0,    0,    0,    cjm   ,"jst"   , "if (SKY = 1) \x9" },          // 55

{ 19,  0, 0,   0, 1,   0,    0,    0,    cjm   ,"jnc"   , "\10CY = 0) \x9" },            // 56
{ 19,  0, 0,   0, 1,   0,    0,    0,    cjm   ,"jc"    , "\10CY = 1) \x9" },            // 57

{ 19,  0, 0,   0, 1,   0,    0,    0,    cjm   ,"jnv"   , "\10OVF = 0) \x9" },           // 58
{ 19,  0, 0,   0, 1,   0,    0,    0,    cjm   ,"jv"    , "\10OVF = 1) \x9" },           // 59

{ 19,  0, 0,   0, 1,   0,    0,    0,    cjm   ,"jnvt"  , "if (OVT = 0) \x9" },          // 60
{ 19,  0, 0,   0, 1,   0,    0,    0,    cjm   ,"jvt"   , "if (OVT = 1) \x9" },          // 61
...
{ 23,  0, 0,   0, 2,   0,    0x7,  0x7,  bjm   ,"jnb"   , "\xa\3\2 = 0) \x9" },         // 70
{ 23,  0, 0,   0, 2,   0,    0x7,  0x7,  bjm   ,"jb"    , "\xa\3\2 = 1) \x9" },         // 71

{ 18,  0, 0,   0, 2,   0,    0,    0x47, djnz  ,"djnz"  , "\3--;\xbif (\3 != 0) \x9" },  // 72
...
{ 20,  0, 1,   0, 0,   0,    0,    0,    ret   ,"reta"  , "return;" },
{ 20,  1, 0,   0, 0,   0,    0,    0,    ret   ,"reti"  , "return;" },
{ 20,  0, 1,   0, 0,   0,    0,    0,    ret   ,"retia" , "return;" },                   // 83

{ 41,  1, 0,   0, 0,   0,    0,    0,    pshp   ,"pushp" , "push(PSW);" },                // 84 push psw
{ 41,  0, 1,   0, 0,   0,    0,    0,    pshp   ,"pshpa" , "pusha(PSW);" },
{ 42,  1, 0,   1, 0,   0,    0,    0,    popp   ,"popp"  , "PSW = pop();" },              // 86
{ 42,  0, 1,   1, 0,   0,    0,    0,    popp   ,"poppa" , "PSW = popa();" },             // 87


{ 1 ,  0, 0,   0, 1,   0x7,  0,    0x47, clr   ,"clrb"  ,  "\4\3 = 0;" },                // 88
{ 1 ,  0, 0,   0, 1,   0xf,  0,    0x4f, clr   ,"clrw"  ,  "\4\3 = 0;" },

{ 2 ,  0, 0,   1, 1,   0x7,  0,    0x67, neg   ,"negb"  ,  "\3 = -\1;" },                // 90
{ 2 ,  0, 0,   1, 1,   0xf,  0,    0x6f, neg   ,"negw"  ,  "\3 = -\1;" },

{ 25,  0, 0,   1, 1,   0x7,  0,    0x47, inc   ,"incb"  ,  "\3++;"  },         // 1++ ?                   // 92
{ 25,  0, 0,   1, 1,   0xf,  0,    0x4f, inc   ,"incw"  ,  "\3++;" },                    // 93
{ 22,  0, 0,   1, 2,   0x7,  0x1f, 0x47, nrm   ,"norm"  ,  "\3 = normalize(\22);" },
{ 2 ,  0, 0,   1, 1,   0x7,  0,    0x47, cpl   ,"cplb"  ,  "\3 = ~\1;" },                // 95
{ 2 ,  0, 0,   1, 1,   0xf,  0,    0x4f, cpl   ,"cplw"  ,  "\3 = ~\1;" },

{ 24,  0, 0,   1, 1,   0x7,  0,    0x47, dec   ,"decb"  ,  "\3--;" },                    // 97
{ 24,  0, 0,   1, 1,   0xf,  0,    0x4f, dec   ,"decw"  ,  "\3--;"  },
{ 22,  0, 0,   1, 1,   0x7,  0,    0x6f, sex   ,"sexb"  ,  "\23 = \1;" },
{ 22,  0, 0,   0, 1,   0xf,  0,    0x7f, sex   ,"sexw"  ,  "\23 = \1;" },                // 100


{ 21,  0, 0,   1, 0,   0,    0,    0,    stc   ,"stc"   ,  "CY = 1;" },                  // 101
{ 21,  0, 0,   1, 0,   0,    0,    0,    clc   ,"clc"   ,  "CY = 0;" },                  // 102
{ 43,  0, 0,   0, 0,   0,    0,    0,    edi   ,"di"    ,  "interrupts OFF;" },
{ 44,  0, 0,   0, 0,   0,    0,    0,    edi   ,"ei"    ,  "interrupts ON;" },
{ 48,  0, 0,   0, 0,   0,    0,    0,    clv   ,"clrvt" ,  "OVT = 0;" },
...
void set_rgsize(OPS *o)
 {
   RST *g;
   uint fend;

   fend = o->fend & 31;    //strip write,sign...
   g = get_rgstat(o->addr);

   if (!g) return;

   if (fend > g->fend) g->fend = fend;
 }
...
void copy_stk_entry(STACK *k, SBK *dest)
{
  int i;
  STACK *d;
  ...
  for (i = 7; i >= 0; i--)
   {
     d = dest->stack+i;
     if (!k->type ) break;
     if (d->sceaddr == k->sceaddr && k->nextaddr-d->nextaddr < 32)
       {
        d->nextaddr = k->nextaddr;
        copy_stk_entry(d,dest->caller);             // copy backwards up the chain
        break;
       }
   }
}
...
void do_stack_back(SBK *s)
{
   // propogate modified stack entry back up the chain.  For 3654 type multiple calls to arg getter(s)
   STACK *k;
   int i;
   ...
   for (i = 7; i >= 0; i--)
   {
     k = s->stack+i;
     if (!k->type) break;
     if (k->modded) copy_stk_entry(k, s->caller);
   }
}
...
void set_emuargs(SBK *s, RPOP *g)
{
  ...
  //   if (g->tgtblk[0] == s->jfrom)
   //    {
   //              #ifdef XDBGX
   //      DBGPRT(1, "JUMP TO %x ", g->tgtblk[0]->start);
      //         #endif
  //     }
     ...
     if (g->tgtblk[0] == s)
      ...
      {
               #ifdef XDBGX
        DBGPRT(1, "Mark emu %x (for %x)", g->tgtblk[0]->start, g->tgtblk[0]->nextaddr);
        #endif
        g->tgtblk[0]->emureqd = 1;
      }
}
 ...
 // ans = 0;
  ...
  if (s->proctype == 1)
    {
     ...
     if (!p->argblk[0]) return 0;

  //   if (o->addr == p->argblk[0]->popreg)
       {
        ans = g->argcnt;
     ...
     set_emuargs(s, p);
     #ifdef XDBGX
     DBGPRT(1, "args %d to %x, target %x", ans, s->start, p->argblk[0]->start);
#endif
     return ans;
    }
  ...
  if (s->proctype == 4)  // emulate
    ...
    //  SPF *f;
      ...
      if (!p->argblk[1]) return 0;           //crash fix ??

 //     if (p->argblk[1]->nextaddr == 0x93e2b)
 //     {
  //        DBGPRT(0,0);
  //    }
     ...
     #ifdef XDBGX
      DBGPRT(1,"argblk[1]=%x %x",p->argblk[1]->start, p->argblk[1]->nextaddr);
      #endif

   //   if (o->addr == p->argblk[1]->popreg)
        {      // register matches stack entry (via reg status)
          o->val |= g_bank(p->argblk[1]->nextaddr);
          ans = o->val - p->argblk[1]->nextaddr;                //orig->nextaddr;
          if (ans > 32)
             {
     #ifdef XDBGX
              DBGPRT(1, "FAIL %x - %x", p->argblk[1]->nextaddr, o->val);
#endif
     ...
     #ifdef XDBGX
   DBGPRT(1,"E%x Add args at %x size %d ", s->curaddr, p->argblk[1]->nextaddr, ans);
   #endif

   scn = p->tgtblk[0];          //real scan
...
//spf should be attached to ptarget here
   ...
   if (get_ftype(p->argblk[1]->nextaddr) == 4 )                 //orig->nextaddr) == 4)
     { // already flagged as args ??
          #ifdef XDBGX
       DBGPRT(1,"ZZZZ Duplicate Args %x", p->argblk[1]->nextaddr);         //orig->nextaddr);
       #endif
  //     k = 0;
     }
   else
    {
      ...
      k = add_aux_cmd(p->argblk[1]->nextaddr, p->argblk[1]->nextaddr +ans-1, C_ARGS, s->curaddr);
      if (k)
        {
...
if (f) DBGPRT(0,"found %d", f->spf); else DBGPRT(0,"NOT found dfdf");
 ...
 DBGPRT(1," spf for %x ", p->tgtblk[1]->start);        //why doesn't [0] work ??

//add link here......lbk->spf       add_link...

    //  set_ftype(p->argblk[0]->nextaddr,4);          //orig->nextaddr, 4);         //only first one ???       //set_ftypedat ??
*/

      if (get_lasterr(CHAUX))            // && k->from == x->curaddr)
        {
                 #ifdef XDBGX
         DBGPRT(1,"ZZZZ Duplicate Args cmd %x", p->argblk[1]->nextaddr);      //orig->nextaddr);              //this must be
         #endif
        }
      else
       {
        ...
        k->size = adn_totsize(vconv(k->start));
...
// but TGTBLK[1]
        ...
        p->argblk[1]->nextaddr += ans;                 //this might stop swop to aux link ?? was originally argblk....
        x->nextaddr += ans;                            // and stack
        x->modded = 1;                                 // mark change
        do_stack_back(s);                              // propgate backwards
       }
    }
...
//     o = c->opr+4;                  // sce op for increment check.
     ...
     #ifdef XDBGX
       DBGPRT(1,"%x pop+inc for %x", s->curaddr, p->tgtblk[0]->start);
     #endif
...
void do_rbt_check(SBK *s, OPS *o, INST *c)
{

  // called only from upd_ram if register has changed in value
  ...
  xadd = o->addr;
  if (o->addr & 1)
      xadd = o->addr-1;        // word equiv in case byte update
  ...
  l = get_rgstat(xadd);
   ...
   if (b)
    {           // entry in chain, probably user command.
      ...
      if (b->usrcmd)
        {
          l->rbase = 1;         //probably redundant, but safety reset....
         #ifdef XDBGX
      ...
      //allow reg to be used a couple of times for boot up

      if (l->rbcnt == 2)
        {
         l->rbase = 0;          //not valid anymore
         b->inv = 1;
        }

      l->rbcnt++;                  //     max of 2 changes

      #ifdef XDBGX
           DBGPRT(0,"Rbase %x = ", add);
           if (o->addr & 1) DBGPRT(0, "via R%x ", o->addr);
           if (l->rbcnt < 3) DBGPRT(1,"%d", l->rbcnt); else DBGPRT(1,"Inv");
...
// all of these are whole read symbols
  ...
  if (ix >= NC(autonames)) return NULL;              // safety check

  if (!get_cmdopt(autonames[ix].mask)) return NULL;  // autoname option not set

  // make name first

  c =  sprintf(strbuf,   "%s", autonames[ix].pname);
  // add address
  c += sprintf(strbuf+c, "_");
  if (numbanks)  c += sprintf(strbuf+c, "%x", (ofst >> 16) -1);   // bank if reqd
  sprintf(strbuf+c, "%04x", nobank(ofst));

  xsym = add_sym(strbuf,  ofst, 0, 7 | C_SYS | C_REN, 0,0xfffff);     // add new (read) symbol, or rename
...
int score_cell(uint start, uint fend, uint rowsize)
{
 ...
 /* from 'x' compare adjacent cells fro a table
 * x -> x-rowsize-1
 * x -> x-rowsize
 ...
 * x-> rowsize + 1     loop 3
 */
  int baseval, adjval, score;
  uint i;
  score = 0;

  baseval = g_val(start,0, fend);

  //row above, 3 cells
  for (i = start-rowsize - 1; i < start-rowsize+2; i++)
    {
      adjval = g_val(i,0,fend);
...
tries = 1;
  ...
  #ifdef XDBGX
  DBGPRT(0, "table %x, %d X %d", start, rowsize, colsize);
  #endif
  score = 0;

  end = start + colsize*(rowsize-1) -1;          // not last row or last col
  start += colsize+1;                            // not first row or first col
...
//mark 'end of stack'  somehow ???  but this is going to go backwards anyway....
 ...
 // base->stack[base->stkptr].stop = 1;        // mark where to stop emulate ?  wrong ?
   ...
   for (i = 5; i < 13; i++)
     {
      if (autonames[i].fendin == isize && autonames[i].fendout == osize)
        {
         new_autosym(i,blk->start);          // add subr name acc. to type
...
//   blk->logdata = 0;        // no data logging (it's where sig found)
 ...
 //  #endif
   ...
   new_autosym(13 +s->v[0] + osign, blk->start);            // add subr name acc. to type
   n = autonames + (13 +s->v[0] + osign);

   f->fendin  = n->fendin;
   f->fendout = n->fendin;
 //  if (osign) f->fendout |= 32;                   // size and sign. done in n
   f->argcnt = 2;
...
//use v[15] ??
 ...
 //  r->enc = enc;
                // enc is always a word
 //  r->ereg  = nobank(s->v[5]);     // 'start of data' base reg

//but this is no good without the START of the args....
//curaddr + opcode size.....

   //k =
 //  get_cmd(CHAUX, r->argsblk->nextaddr, 0);
 //  if (k)
 //  {
 //      DBGPRT(0,0);        Not here.....
 //  }
...
FKL *l;
 ...
 #ifdef XDBGX
  DBGPRT(1,"  spf = %x, adt = %x", s, a);
#endif
  size = a->cnt;            //keep overall size
       ...
       for (j = 0; j < s->argcnt; j++)
         {
           d = s->pars + j;
           g = get_rgstat(d->reg);
           d->dfend = g->fend & 15;            // fiddle here for max words ....
   ...
   while (ix < 3)
    {
     o = c->opr+ix;
     r = get_rgstat(o->addr);
     if (r && r->arg) set_rgsize(r, o->fend);
     ix++;
    }
  ...
  /******************* multiple address mode, by subtype *****************
    ...
    ******************************************************/
     ...
     //       o->rgf = 0;                                // not a register anymore but this breaks rbases
            ...
            set_rgsize(o);                           // keep size
            cp->val = o->addr;                       // save address in the op copy as value
            cp->fend = 15;                           // val in copy is now a WORD
            set_rgsize(cp);                          // for read ?
          }
...
//if (s->curaddr == 0x977c2 && s->proctype)
//{
 //   DBGPRT(0,0);
//}
...
void add_spf_register (RPOP *p, uint dest, uint sce, uint popaddr)
{
  ...
  if (!get_spf(vconv(p->argblk[1]->nextaddr)))
    {
        f = add_spf(vconv(p->argblk[1]->nextaddr),10,1);
        p->fblk = f;
             #ifdef XDBGX
        DBGPRT(1,"add spf 10 to %x", p->argblk[1]->nextaddr);      //p->tgtblk[0]->start);
        #endif
    }
...
//if (s->curaddr == 0x977cc && s->proctype == 4)
//{
//    DBGPRT(0,0);
//}
 ...
 // indirect lookup done only if proctype set, so print still works
...
#endif
   //    if (s->proctype == 4) k = p->tgtblk[1];
   //    if (s->proctype == 1) k = p->tgtblk[0];
...
//if (s->curaddr == 0x94880 && s->proctype)
//{
//    DBGPRT(0,0);
//}
   ...
   if (s->proctype == 4)         // emulate mode
    {
     ...
     #ifdef XDBGX
         DBGPRT(1,"%x (ldx) R%x(%x) set as arg for %x",s->curaddr, o->addr,o->fend, p->tgtblk[1]->start);
         #endif
...
// if unclosed spf of 10, flag the jump
 ...
 if (f) {
  ...
  //   type = get_stack(s, 0, &x, 0); ---- doesn't work
...
// s->stkptr  ?? if (get_stack(s, 0, sc, 0) < 3);   sc is entry with popreg set.        s->stkptr ?
...
// basic address stuff first................
  ...
  if (bankeq(addr, caller->curaddr))                      // bank check
     {                                                    // in same bank
      ...
      // backward jump, scanned already - is a loop
      if (jump < 0 && get_ftype(addr) == 1)
        {
          if (type == J_COND) ans = 2;
          if (type == J_STAT && addr > caller->start)  ans = 2;   //only within same block
        }
...
SBK *do_cond_scan(SBK *caller, INST *c)          //, int type)
{
  ...
  //newscan =
  add_scan (caller->chn, addr, 0, J_COND, caller);    // where cond continues
...
SBK *pshp(SBK *s, INST *c)
 {
    ...
    s->stkptr++;
    k = s->stack+s->stkptr;       //previous entry
    s->stkptr--;

    if (s->curaddr == k->sceaddr)
      {
          #ifdef XDBGX
...
//push HW correct format
 ...
 if (s->curaddr == s->start && k->type)
   {
     pp = (k->sceaddr >> 6) & 0x3c00;        //  get caller's bank
   }
 else
 ...
 put_stack(s, 3, pp, 0);
         ...
         #ifdef XDBGX
      DBGPRT(1,"%x PUSHP %x = %x,%x,%x",  c->ofst, pp, s->psw, pp >> 10, (pp >> 8) & 3);
...
//if (s->curaddr == 0x92400 && s->proctype == 1)
//{
//    DBGPRT(0,0);
//}
...
//  end = fl->flength;
  ...
  find_cpyrt(binfbuf);    // copyright and calibration text, after banks done
...
void do_calls(SBK *s, SFIND f)
{
 ...
 uint jx, ix, ofst;
      ...
      #ifdef XDBGX
        DBGPRT(1,0);
        DBGPRT(0,"Base calls for blk %x", s->start);
      #endif
 ...
 while (1)            //n)
     {
      ...
      // n MUST be wrong - delete it.
      // but keep if higher than addr or user cmd
...
void extend_func(LBK *k, uint ix)
{
  ...
  uint eadr, max,  rsize, adr, vmatch;     //size
  int val, lastval, eval;
  ADT *a, *b;
  LBK  *n;
...
// }
 ...
 // size = bytes(a->fend);
  ...
  eval  = get_endval(a->fend);              // end value



  // WRONG !!! probably need get_endval.........


  adr   = k->start;

  // need to check next command as well....
  val = 0xd0000;                       // impossible value for start
  vmatch = 0;
  eadr = k->end;

  while (adr < max)                      // check input value for each row (UNSIGNED)
    {
     adr += rsize;
     lastval = val;
     ...
     // check if value drops - max 4 rows of same value ??
     //need to mod this for END values.
     if (val >= lastval) vmatch++;
...
max = adr +  (bytes(rsize) * 10);              //allow 10 rows extra

if (max > eadr) max = eadr;
...
LBK *k;
 ...
 while ((k = (LBK*) get_next_item(CHCMD,&ix)))
     ...
     //ix points to next item.....
 // for (ix = 0; ix < chcmd.num; ix++)
   {
  //   k = (LBK *) chcmd.ptrs[ix];
     if (!k->usrcmd)
       {
        if (k->fcom == C_FUNC)  extend_func(k,ix);
        if (k->fcom == C_TABLE) extend_table(k,ix);
       }
   }
...
// scan tabs ??
 ...
 //  scan_lpdata();
  ...
  do_functabs();                // find structs via funclu and tablu calls

// find_funcs();

  extend_functabs();              //may be better to try to find all functions FIRST....
