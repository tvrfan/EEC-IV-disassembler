 ...
 *
 * 806x bit numbering bit 0 is least significant (rightmost), bit 7(15 etc) is most sig.
 *
 *****************************************************************************
...
#define SADVERSION "5.0.8 Alpha"
#define SADDATE    "31 Jan 2024"
...
typedef struct
{
 ...
 char  *barename;       // bare bin file name (root binary name)
 char  *sadpath;        // path to SAD exe (argv[0])
 char  *filepath;       // path from filename (argv[1], typically)


 uint  pathsize[7];     // strlen+1
 uint  namesize[7];     // strlen+1
 uint  baresize;        // barename bin strlen+1
 uint  sadpsize;        // sadpath strlen+1
 uint  filpsize;        // file path size
...
uint lastix;                                   // last search index, for shortcuts and 'get_next_' or for duplicate ?
uint lasterr;                                  // error number, for user commands etc
...
int   (*chprt ) (struct chain * , uint, uint,  uint (*prt) (uint,cchar *, ...));     // print item (command errors, msg etc)
} CHAIN;
...
typedef struct          // operand storage, max 4 for each instruction
{
 ...
 uint prt : 1;          // printed once ?? for use with divide....or prt cnt ??
} OPS;
...
// is it worth having a parallel inst for printing ???
...
typedef struct inst
{
  ...
  OPS   opr[5];            // 5 operands.  [0] for indirect and indexed. [4] is saved original [1],
...
//24
  ...
  uint  sign     : 1;      // sign bit prefix (0xfe) for sigs mainly
  uint  jswp     : 1;      // used to swop logic when printing.  (goto -> {)
...
typedef struct jlt
{
 ...
 // uint   swl      : 1;     // swop 'if' logic sense for or bracket same as obkt
...
typedef struct dp
{
  ...
  uint dfend       : 6;       // 0-31 for size  (no read or write? sign ?)
...
{
  uint start     : 20;      // for SBK find,  psw for pushp
  uint popreg    : 10;      // register popped
  uint nextaddr  : 20;      // or sceaddr for psp ??

  uint  curaddr  : 20;      // where this entry was added from.... for duplicate checks.  read from ??)
  uint  type     : 4;       // 1 - call (scan), 2 - immd push (scan)   3 - pushp (start=PSW)
...
typedef struct rgbase
 {

  uint valu    : 20;      // rbase address
  uint rbcnt   : 3;       // changed count, for rbases
  uint invrbt  : 1;       // invalid rbase register
 } RBST;
...
typedef struct regstx
 {
  ...
  uint fend    : 5;       // size 0-31 for arg sizes (sign ?)
  uint argcnt  : 4;       // popped reg increments  -> args present.
  uint rbcnt   : 3;       // changed count, for rbases
  uint invrbt  : 1;       // invalid rbase register
  ...
  uint fendlock : 1;      // lock size (ldzbw and ldsbw)
 } RST;
...
typedef struct rpop
{
 SBK *tgtscan;       //blk[2];         // real [0] and emu [1] subr scan block.  (i.e. the call before nextaddr)
 SBK *tgtemu;
 SBK *argscan;              //blk[2];         // real [0] and emu [1] block where nextaddr is (where the args actually are, and caller of subr)
 SBK *argemu;
...
// fend below is same as ADT fend , but with more markers as well. see definition in core
...
typedef struct            // OPCODE definition structure
 {
  ...
  uint  pswc    : 6;             // op changes the PSW (for conditional ifs) 6 bits if done by flag
  uint  nops    : 2;             // number of ops   (max 3)

  uint  fend    [3];             // opsizes as 0-x (field end), +0x20 if signed, +0x40 if write, use as true/false extras for jumps
...
{                         // variable part of sig for holding mathces
 ...
 uint  type;               // sig type, for duplicate checks
 uint  hix;               // signature handler index (calls subr[hix])
 uint  done   : 1 ;       // processed marker - for debug really
 uint  novlp  : 1;        // cannot overlap with others (itself ?)
 uint  v[NSGV];           // 31 saved values.  v[0] is a param for handler subr
...
typedef struct              // command holder for cmd AND COMMENT parsing
{
  ...
  int  adtsize    ;       // total size of ADT  (bytes)
  int  mcalctype  ;
  int  mfunc      ;           // math built in function
  int  strix      ;       // special string index
 ...
 ********************************/
 ...
 typedef struct drs               // command definition
 {
  ...
  uint  maxadt  : 5 ;             // max main ADT levels (0- 31)
              // sub levels ?
  uint  dsize   : 3 ;             // default cmd size, bytes
