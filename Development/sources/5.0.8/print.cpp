...
//from 0 to 5  bits 1 and 2 skipped
// char pswflags[] = {ST, C && !Z, N || Z,   C, VT, V, N, Z ,     //st V VT z n v


//   Z N V VT C ST    in order of manual.
...
uint prtfl(cchar *fmt, float fv, int pfw, int pfd, uint (*prt) (uint,cchar *, ...))
 {
  ...
  // note that pfd is regarded as part of pfw.
     ...
     if (x->bprt)
       {
        ...
        prtaddr(x->minromadd,0,prt);
        prt(0,"  ");
        prtaddr(x->maxromadd,0,prt);
...
void pl_spf (CHAIN *c, uint flags, uint ix, uint (*prt) (uint,cchar *, ...))
...
void pl_rbt(CHAIN *x, uint flags, uint ix, uint (*prt) (uint,cchar *, ...))
{
...
void pl_adl(CHAIN *x, uint flags, uint ix, uint (*prt) (uint,cchar *, ...))
{
...
void pl_cmd(CHAIN *x, uint flags, uint ix, uint (*prt) (uint,cchar *, ...))
{
  LBK *c;
...
void pl_bnk(CHAIN *x, uint flags, uint ix, uint (*prt) (uint,cchar *, ...))
{
  BNKF *c;
...
void pl_psw (CHAIN* x, uint flags, uint ix, uint (*prt) (uint,cchar *, ...))
{
  PSW *r ;
...
//need pfw to line up columns !!!!
      ...
      default:          //but no symname find ????
        ...
        pstr(0,"%*x", pfw, h->total.ival);                  // *pfw, testdefault hex  (case 0 and 1
...
void pl_sub (CHAIN *c, uint flags, uint ix, uint (*prt) (uint,cchar *, ...))

{
 ...
 uint go;
        ...
        if (go)
          {
           if (prt == wnprt) {prt(0,"sub  ");  prtaddr(s->start,0,prt);}
           else
             {
...
void pl_sym (CHAIN *x, uint flags, uint ix, uint (*prt) (uint,cchar *, ...))
{
 SYM *t;
 uint pbkt;
  ...
  if (prt != wnprt) pbkt = 1;        //always print for debug.
   ...
   if (!t->noprt)
     {
      if (prt == wnprt)
         {
          prt(0,"sym ");
          prtaddr(t->addr,0,prt);

          if (t->rstart)
           {
            prt(0,"  ");
            prtaddr(t->rstart,0,prt);
            prt(0,"  ");
        ...
        {         // debug
         prt(0,"[%3d] sym %4x ", ix, t->addr);
         prt (0," %5x %5x ", t->rstart, t->rend);
         prt(0, " (%2d)", t->symsize);
         prt(0," mask=%4x",t->fmask);
         if (t->pbkt)  prt(0," bkt"); else prt(0,"    ");
         if (t->pbits) prt(0," pbt"); else prt(0,"    ");
         if (t->whole) prt(0," whl"); else prt(0,"    ");
        }
     ...
     //      if (t->fend & 32) prt(0, " S");         // signed ??

     if (t->fend & 0x40) prt(0, " W");

     if (t->pbits || prt != wnprt)
        {
          prt(0," B %d" , t->fstart);

          if (t->fend != t->fstart || prt != wnprt) prt(0, " %d", t->fend & 0x1f);
        }
...
// in separate ops
 ...
 if (o->rgf)
    {
     if (!o->addr && (o->fend & 64) == 0) pstr (0,"0");           //zero reg (read only)
 ...
 if (o->adr)
    {           // an address - sort bank if necessary
...
if (o->off)
    {            //offset address - signed
     ...
     if (!get_numbanks() || nobank (v) < 0x400) v &= get_sizemask(15);
      ...
      {
        if (x->bank)  v |= ((x->bank-1)<< 16);   // put inst bank in (prefix)
        else v |= ((x->scan->datbnk-1)<< 16);    // put scan bank in (databank)
      }
...
//need to get OPC entry ??
  ...
  int i, flags;
   ...
   p = get_opc_entry(x->opcix);     // and index 0-2 so ix-1

   flags = p->fend[ix-1];

   if (flags & 0x100)           //   ix >= 0x11 && ix <= 0x13)
      {                 // extra size/sign requested
       if (!x->opr[ix].sym  && x->opr[ix].rgf)
         {
           i = x->opr[ix].fend;
           if (i & 32)  pchar('s');
           i &= 31;
           if (i < 8)  pchar('y');
           else if (i < 16)  pchar('w');
           else if (i < 24)  pchar('t');
           else pchar('l');
        //   pchar('_');
  ...
  if ((flags & 0x400) && o->prt)
   {
     o->addr += bytes(o->fend);
   }

  if ((flags & 0x800) && o->addr == 0) {}
  else
  {
  if (o->bkt) pchar ('[');
  p_sc (x, ix);
  ...
  if (o->bkt)  pchar (']');
   ...
   if (p->fend[ix-1] & 0x800)
     {               // add carry flag
       cchar *t;
       t = p->sce + 2;      // gets op

      if (o->addr)
        {  // not reg zero, add operator
         pchar (' ');
         pchar(*t);
         pchar(' ');
       }
...
/*
void p_opskp (INST *x, cchar **tx)
{
  //  if op is zero, skip over, and next operator too
  uint i;
...
void cmtwrapcheck(CPS *c)
{
  char *t;
  uint col;
  // check if next 'word' would wrap after a space

  if (*c->cmpos != ' ' && *c->cmpos != ',') return; // not at space or comma

  if (c->cmend - c->cmpos > 40) return;

  t = c->cmpos+1;         // next char
  while (t < c->cmend)
   {
     if (*t == ' ' || *t == ',' || *t == '\n' || *t == '\r' ) break;
     t++;
...
void parse_cmnt(CPS *c, uint *flags)
{
  ...
  SYM *x;
   ...
   if (*c->cmpos != '\\' ) p_pad(CMNTPOSN);  //no special chars, tab to cmnt position
    ...
    { // special char. if followed by a 'n', assume it's for NEXT code line, not for a split
      if ((*flags) & 2 && *(c->cmpos + 1) == 'n') return;
    }
   ...
   while (c->cmpos < c->cmend)
    {
     if ((*flags) & 4) break;         //exit is flagged, for split comments with a '|'
                    ...
                    if (*c->cmpos == ':')
                     {
                      ...
                      ans = getpd(c,2,1);                 //bit number - only one for now...
         ...
         //      case 't' :
          ...
          //       c->cmpos = c->cmpos+chars-1;                                   // skip the sequence + whatever read
           ...
           //      chars = gcol;       //current col
              ...
              case '|'  :        // multiple comment lines / split comment for bits etc.
                 if ((*flags) & 2) (*flags) |= 4;
                 c->cmpos++;
                 break;
...
uint pp_timer(uint ofst, LBK *x)
{
  ...
  sym = 0;
  a = get_adt(vconv(x->start),0);              //,1);

  if (!a)  a = add_adt(x,0);              //,1);     // safety
  ...
  if (!val)
    {
      pp_hdr (ofst,"byte",1);    // end of list
      pstr(0,"# Terminator", val);
      return xofst;
     }

  a->fnam = 1;                      // TEMP but works for names !!

  cnt = (val & 1) ? 3 : 1;           // long or short entry
  pp_hdr (ofst, "timer", bytes(a->fend) + cnt);
  pstr(0,"%2x, ", val);
  val = g_val (xofst,0, a->fend);
//val = fix_sbk_addr(val);                    //2nd entry.
  if (a->fnam) sym = get_sym(val,0,7,xofst);       // syname AFTER any decodes
...
//val = fix_sbk_addr(val);
     if (a->fnam) {sym = get_sym(val,bit,0,xofst);
     if (!sym) sym = get_sym(val,0,7,xofst); }            //bit _ no longer correct ???
    if (sym) pstr(0,"%s, ",sym->name);
...
uint do_bitnames(INST *c, uint addr, uint vmask, uint pmask, cchar *op, uint *list)
 {
    //do named fields
      ...
      if (symmask & pmask)
        {
          ...
          if (pmask)   pp_comment(c->ofst+1,2);               // check for comment THIS BUGGERS UP the list !!!
        }
       i++;
    }
  return pmask;
...
uint do_bitdeflt(INST *c, uint addr, uint vmask, uint pmask, cchar *op, uint *list)
   {
      //  Bx_Rn  printouts.
     ...
     // [1] is write default, list [0] is read

      if  (list[1] < x->num) ix = 1;  else ix = 0;

      if (list[ix] < x->num)  d = (SYM*) x->ptrs[list[ix]]; else d = 0;
...

{
  ...
  if (!o->addr) return;                        // ignore if R0 (but what if via a jne or something ???)

  vmask = c->opr[1].val;                       // value of read/write  for bits OR and AND

  o = c->opr + 2;                              // mask is op[1], but sym source is op[2]

  list = get_syms(o->addr, 0, o->fend, c->ofst);  // size via o->fend (write) ....

  x = get_chain(CHSYM);

 if (list[2] > x->num) return;
...

{
  ...
  //if it's an3b or an3w, need to mask op[2] in here...........split it off ???
  ...
  if (sig == 12 || sig == 1)
    {                                            // ldb, ldw, clr. etc
      pmask = fmask(0,c->opr[1].fend);           // for all bits in operand
    }
  ...
  if (sig == 5)
    {                                            // AND. Reverse print mask.
       pmask = ~pmask;
       pmask &= fmask(0,c->opr[1].fend);         // cut back down to right size
    }
  ...
  if (list[2] > x->num)
    {
      if (sig == 12 || sig == 1)  { mfree(list,70*sizeof(uint)); return;}
      if (__builtin_popcount(pmask) != 1)  { mfree(list,70*sizeof(uint)); return; }     // at least one sym must match to do anything, but single bit OK
    }

  pmask = do_bitnames(c, o->addr, vmask, pmask, op, list);

  *tx = 0;

  //end of list, but pmask may still have bits set, so do defaults

 if (pmask && (sig == 5 || sig == 9))      //not for clr or ldx
    {       //and,  OR use Bx_Rn.........
      pmask = do_bitdeflt(c, o->addr, vmask, pmask, op, list);
    }

    //  if (pmask)   pstr (0, " **P %x **", pmask);  /// DEBUG but what to do ????
...
void shuffle_ops(cchar **tx, INST *c, int ix)
{
  ...
  while (ix < 3)
    ...
    //need to scan for sign bits ??


  if (tx) *tx = "\3 = \1;";             //  same as ldx pseudo string
}
...
int find_last_psw(INST *c, INST *out)
 {
   ...
   // This is used by carry and ovf as well
    ...
    chkclc = 0;
    if (c->opcix > 61 && c->opcix < 64) chkclc = 1;  // JLEU/JGTU 62, 63
    if (c->opcix > 55 && c->opcix < 58) chkclc = 1;  // JC/JNC    56, 57

    // what about a popp ?? but how the hell to find THAT ??
    ...
    while (!ans)
     {
      ...
      /* {
        // CALL - if tracing subrs find jump and go to end of subr
        JMP *j;
        SUB *sub;
...
void prt_jump(INST *c, uint swop, cchar **tx)
{

//     note that *tx at this point will always be last char ??   j =
...
// if (swop == 1)
// {  //swop opcode if goto versus {

     /*   if ((jf & 1) && *tx)
         {                             // invert jump logic for source printout
           if (c->sigix == 19 inx > 53 && inx < 72)   // all conditional jumps (except djnz)
           {
             const OPC *s;
...
//if (j->jor)   pstr(0," ** OR **");
    ...
    //   if (j->jtype == J_ELSE) { pstr (0, "} else { "); return;}
           ...
           //    if (j->cbkt)  pstr (0,"C ");   // [if] case
               ...
               if (j->obkt  && c->sigix != 18)  pstr (0,"{");   // [if] case
...
break;
               ...
               */
                 ...
                 if (j->retn)   {pstr (0,"return;"); *tx = 0; return;}  // static jump to a return
...
*tx = 0;         // safety - no more psuedo code after jump or bkt

}

/*
void prt_flg(INST *c, cchar **tx)          //, int ainx)
{
...
//    if (!j) return ;
  ...
  j = get_fjump(c->ofst);
   ...
   opl = get_opc_entry(ix);  // mapped to 'correct' instance for print
...
void pp_cond_jmp(INST *c, cchar **tx)
{
   ...
   if (j && j->obkt) ix = c->opcix ^ 1; else ix = c->opcix;



//djnz needs a special.............
   ...
   *tx = opl->sce;      // select correct version
   ...
   t = *tx+1;    // skip the '\7', t now points to operator string

   ans = find_last_psw(c,&psinst);  // find for previous ops (psinst) or zero

   pstr (0,"if (");

//if (!ans)
// { use bitmap to print check state




   if (! psinst.opr[3].fend)                     // no write, so is a compare type
     {      // compare found, style R2 = R1
      if (!nobank(psinst.opr[2].addr))
       {    // R2 = 0
         if (!nobank(psinst.opr[1].addr) && ans)
          {    //  0 = 0  a patch ??
             //so t has correct op string...........if theres a ! it's false,
             // if there's a '=' it's true............
             ix = 0;          // false, default
             while (t)
              {
               if (*t == '!') break;
             ...
             if (!ix) pstr(0,"false");  else pstr(0,"true");
         ...
         else
          {
           ...
           pstr(0," %s ", t);                        // safe as all \7 have just the op
   ...
   if (psinst.opr[3].fend)
     {            // not a compare
      if (!nobank(psinst.opr[3].addr))
        {
         // target  is R0, style  "(2 op 1) = 0"

         if (nobank(psinst.opr[2].addr))
           {  // print only [1] and op if zero register in [2]
            const OPC *opx;
            opx = get_opc_entry(psinst.opcix);               // source code of psw op
            x = strchr(opx->sce, '=');
            if (x)
             {  // do orig opcode in brackets
              x += 3;                               // skip first operand
              pstr(0,"(");
              p_opsc(&psinst,2);                    // orig operand 2
              while (*x > 0x1f) pchar(*x++);        // orig operator
              p_opsc(&psinst,1);                    // orig operand 1
              pstr(0,")");
             }
           }
         else
           {
             p_opsc(&psinst,1);                    // only operand 1 set?
           }
         pstr(0," %s ",t);                      // orig compare op
         p_opsc(0,1);                         // orig compare value
        }
      else
       {   // no compare real wop , style is if writeop == 0
        p_opsc(&psinst,3);
        pstr(0," %s ",t);
        p_opsc(0,1);
       }
    ...
    pstr(0,") ");
    prt_jump(c,0,tx);        //*tx = scespec[3] -1; //opctbl[JMPIX].sce - 1;             //set \9 for 'goto' processing - or call subr....NO !! not with return extras
}
...
// possible opcodes add,shift R, shift L, cmp, inc, dec, neg, sub
   ...
   if (j && j->obkt)
      {
        rev ^= 1;               //flag
        opl = get_opc_entry(c->opcix ^ 1);     //get alternate
        *tx = opl->sce;
      }

   pstr (0,"if (");

   // get last psw changer  changes following logic
   ...
   if (opl->sigix == 7)
        {
          ...
          p_opsc(&psinst,3);  // last write op
          pstr(0," %s ", rev ? ">=" : "<");
          pchar('0');
          pstr(0,") ");
          prt_jump(c,0,tx);         //*tx = scespec[3] -1;     //opctbl[JMPIX].sce - 1;   //jump sce code to follow this - allow for increment
        }

  if (opl->sigix == 10)
        {
          // after a compare. Same as subtract, but no write
          // JNC -> JLT (67) JC -> JGE  (66)

          if (valid_reg(psinst.opr[2].addr) == 2)
            {   // jc, jnc after a cmp with zero, so swop operands,
              p_opsc(&psinst,1);                   // operand 1 of compare
              pstr(0," %s ", rev ? "<=" : ">");
              p_opsc(&psinst,2);                   // operand 2 of compare (zero)
            }
   ...
   if (opl->sigix == 6  || opl->sigix == 25)
        {       //  add or inc STANDARD CARRY -  have write
          p_opsc(&psinst,3);
          pstr(0,"  %s", rev ? ">" : "<=");
          pstr(0," %x) ", get_sizemask(psinst.opr[3].fend));
          prt_jump(c,0,tx);
        }
   ...
   if (opl->sigix == 3 || opl->sigix == 4)
        {   //  shl (3)   or shr (4)
          ...
          // check no of shifts is imd, otherwise return 0=0 style.
           ...
           if (opl->sigix == 3)
            {   //shl
             ans =  (psinst.opr[3].fend & 31) + 1;       // number of bits
             ans -= psinst.opr[1].val;                   // shift from MSig
...
void prt_jswp(INST *c, cchar **tx)
{
  // swop opcodes id swop marked in jump
...
void fix_sym_names(cchar **tx, INST *c)
{
  ...
  // op 0 (offset) can be a base address so check for symname
  ...
  if (c->opcsub == 3)           //indexed, use address in cell [0] for sym
     ...
     if (c->sigix == 13 || c->sigix == 15) o = c->opr+3;   //stx and pop can have indexed,indirect write
     else o = c->opr+1;

     if (valid_reg(o->addr) == 2) o->val = 0;            // zero register safety

// NB. can't use valid reg below as it returns 0 for reg 1 as well.....
...
//        o->addr = o->val + c->opr->addr;                 // add [0] to get true address
        ...
        s = get_sym(o->addr, 0,7,c->ofst);               // and sym for calc'ed address
  ...
  if (c->opcsub == 1)       // immediate - check opcodes etc. for removing symbol names
     {
      ...
      if (valid_reg(c->opr[2].addr) == 3 && c->numops > 2)   c->opr[1].sym = 0;   // sfrs not normally syms +-
      // sub ??
  ...
  if (opl->nops > 2)
    {    //rbase and R0
     if (c->opr[1].rbs && !c->opr[2].addr)  shuffle_ops(tx,c,2);
    ...
    if (!c->opr[2].addr)   shuffle_ops(tx,c,2);      // x+0, drop R0

    if (!c->opr[1].addr)   shuffle_ops(tx,c,1);      // 0+x, drop R0
...
// other opcodes or registers ?? may need another subr with more rules (IO_timer..)
...
//  preset_comment(ofst);  // begin and end reqd ??
 ...
 //   if (j && j->swl) c->jswp = 1;       // put swop logic here ???
    ...
    //    shift_comment(c);               // add comment to shifts with mutiply/divide  - if (curinst.opcix < 10 && ops->imd)
       ...
       //numbers letters are 0x20 to 0x7f

       while (tx && *tx)
        {

         switch (*tx)   // oct 1-12 used (0x1 0xc), oct  20 for sign, (0x10) oct 30 prob OK (0x18) oct 40 0x20 is SPACE, then oct 200, 0x80..
          {
     ...
     //        break;
           ...
           case 7:                  // all conditional jumps with get-last-PSW for operands
             pp_cond_jmp(c, &tx);
             break;

           case 8:                 // carry  JC and JNC        //becomes other flags too ?? not sure....
             prt_carry(c, &tx);
             break;

           case 9:
             prt_jump(c, 1, &tx);   // general goto (+ else,and,or, bracket etc) and swop codes ??
             break;

// 10 is line feed, 13 is CR

           case 10:
             pp_comment(c->ofst+1,2);               // check for comment
             p_indl();             // \n  (= \xa)  print new line with indent (DJNZ and DIV) Comments ???
             break;

           case 11:
             prt_jswp(c, &tx);     // \xb - bit and flag jumps   JB, JNB
             break;

// 12,14,15,16 onwards to 31.
     ...
     {      // subr arguments required even without source code option
     // use c->sigix == 17 insteadf ??
      if (!get_cmdopt(OPTSSRC) && c->sigix == 17) // (c->opcix == 73 || c->opcix == 74 || c->opcix == 78 || c->opcix == 79))          // CALL opcodes
         {
           p->nextaddr += pp_subargs(c,p->nextaddr);
         }
     }
