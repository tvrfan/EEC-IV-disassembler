    ...
    if (s->stack[i].type)
     {
      ...
      if (x->type == 1 || x->type == 2)
        {
         DBGPRT(0,"{%d} S%x N%x ^%x", x->type, x->start, x->nextaddr, x->sceaddr);
        }

      if (x->type == 3)
        {
                      //   psp = x->psp;
          DBGPRT(0,"{P} %x ^%x",x->start, x->sceaddr);
...
void DBG_sbk(cchar *t, SBK *s)
{
 ...
 if (s->caller) DBGPRT(0," Caller %x", s->caller->nextaddr);
...
// now for multiple queues
   ...
   //     DBGPRT(0, "%lx(%2d) -> %lx(%2d)", l->keysce,l->chsce, l->keydst, l->chdst);
    ...
    if (l->chsce == CHMATHX)   DBGPRT(0, " 0");       //, ((MATHX*)l->keysce)->cnt);
    ...
    if (l->chdst == CHMATHX)   DBGPRT(0, " 0");      //, ((MATHX*)l->keydst)->cnt);
     DBGPRT(1,0);
    ...
    for (i = 0; i < 8; i++)
     {
      ...
      {
         DBGPRT(0,"%x ", i);
         DBGPRT(0,"tgt  %x %x", p->tgtblk[0], p->tgtblk[1]);
         DBGPRT(0,"arg  %x %x", p->argblk[0], p->argblk[1]);
