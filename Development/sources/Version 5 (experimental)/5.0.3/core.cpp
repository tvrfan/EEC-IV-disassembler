...
* 8065 only, updates PSW,  number of ops, show_sign,  3 op sizes (1,2,3 as above),
...
{ 8 ,  1, 0,   0, 2,   0x7,  0x7,  0x4f, mlx   ,"ml2b"  , "\23 = \22 * \1;" },           // 14
{ 8 ,  0, 0,   0, 2,   0x27, 0x27, 0x6f, mlx   ,"sml2b" , "\23 = \22 * \1;"},
{ 8 ,  1, 0,   0, 2,   0xf,  0xf,  0x5f, mlx   ,"ml2w"  , "\23 = \22 * \1;"  },
{ 8 ,  0, 0,   0, 2,   0x2f, 0x2f, 0x6f, mlx   ,"sml2w" , "\23 = \22 * \1;"},
...
{ 6 ,  0, 0,   1, 2,   0x7,  0x7,  0x47, add   ,"adcb"  , "\3 += \31 + \30CY;" },            // 42  x31 ->x30 is skip if zero
{ 6 ,  0, 0,   1, 2,   0xf,  0xf,  0x4f, add   ,"adcw"  , "\3 += \31 + \30CY;" },
{ 7 ,  0, 0,   1, 2,   0x7,  0x7,  0x47, sbt   ,"sbbb"  , "\3 -= \31 - \30CY;" },
{ 7 ,  0, 0,   1, 2,   0xf,  0xf,  0x4f, sbt   ,"sbbw"  , "\3 -= \31 - \30CY;" },
...
{ 15,  0, 1,   0, 1,   0xf, 0,    0x4f,  popw  ,"popa"  , "\3 = popa();" },

{ 19,  0, 0,   0, 1,   0,    0,    0,    cjm   ,"jnst"  , "\xaSKY = 0) \x9" },          // 54  to 71 cond jumps
{ 19,  0, 0,   0, 1,   0,    0,    0,    cjm   ,"jst"   , "\xaSKY = 1) \x9" },          // 55

{ 19,  0, 0,   0, 1,   0,    0,    0,    cjm   ,"jnc"   , "\10CY = 0) \x9" },            // 56  (\10 = 8)
{ 19,  0, 0,   0, 1,   0,    0,    0,    cjm   ,"jc"    , "\10CY = 1) \x9" },            // 57

{ 19,  0, 0,   0, 1,   0,    0,    0,    cjm   ,"jnv"   , "\xaOVF = 0) \x9" },           // 58
{ 19,  0, 0,   0, 1,   0,    0,    0,    cjm   ,"jv"    , "\xaOVF = 1) \x9" },           // 59

{ 19,  0, 0,   0, 1,   0,    0,    0,    cjm   ,"jnvt"  , "\xaOVT = 0)" },          // 60
{ 19,  0, 0,   0, 1,   0,    0,    0,    cjm   ,"jvt"   , "\xaOVT = 1)" },          // 61
...
{ 22,  0, 0,   1, 1,   0xf,  0,    0x7f, sex   ,"sexw"  ,  "\23 = \1;" },                // 100
  ...
  l = get_rgstat(xadd);
   ...
   if (b)
    {           // entry in chain, probably user command.
      ...
      // allow reg to be used a couple of times for boot up, but register - register is not valid
      // should that be anything not immediate ??? ad3w rbase+ x somewhere....


      if (!c->opcsub == 0) l->rbcnt = 3;
...
eadr = adr +  (bytes(rsize) * 10);              //allow 10 rows extra
...
/*
if (rsize == 4)
  {          //word func - check for table dim style (top byte only)
   int lval;
   uint chg;
