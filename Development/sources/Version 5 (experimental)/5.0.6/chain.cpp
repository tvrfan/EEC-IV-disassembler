...
i = lfend % 8;        //  remainder
  ...
  if (fstart == 0 && i == 7)  s->whole = 1;

  //   {         // 'whole' or default symbol
  //     if (lfend == 7 || lfend == 15 || lfend == 24 || lfend == 31) s->whole = 1;
  //   }
...
uint *get_syms(uint add, uint fstart, uint fend, uint pc)
{
  ...
  list = (uint*) mem (0,0, 70*sizeof(uint));  //(SYM**) &scores;           // overlay SYM* array onto scores array
  ...
  while (ix < chsym.num && num < 70)
    {
     ...
     if (pc >= t->rstart && pc <= t->rend)
       {         // within sym range, put deflt read in [0] , deflt write in [1]
        ...
        if (t->whole)
         {         // a 'whole' symbol byte or word, keep longest one declared, read or write
           if (t->fend & 0x40) z = 1; else z = 0;

           if (list[z] > chsym.num) list[z] = ix;           // new match
           else
            {
              s = (SYM*) chsym.ptrs[list[z]];                 // current default
              if (t->fend > s->fend) list[z] = ix;            // replace if t is bigger
            }
         }
        else
         {
    ...
    for (i = 2; i < 70; i++)
        { // check each sym for best match
         ...
         if (s->fstart == t->fstart && s->fend == t->fend && s->addr == t->addr)
           {
            break;         //perfect match (includes read/write)
           }
        }
