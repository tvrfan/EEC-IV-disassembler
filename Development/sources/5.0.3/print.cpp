...

{
  ...
  if (c->opcix > 31 && c->opcix < 34)  t = "^="; else t = "=";    // xor
  ...
  if (sig == 5 || sig == 1)
    {                                            // AND and CLR . Reverse print mask
        pmask = ~pmask;
    }
       ...
       if (d->fmask & pmask)
        {
          ...
          pstr(0,"%s %s %x;", d->name, t, (vmask & d->fmask) >> d->fstart);
...
void fix_sym_names(cchar **tx, INST *c)
{
  ...
  //need to check this for [3] as well as [1] for stx and pop....
   ...
   //     pstr("|%d ", indent);         // experiment, for lining up and new brackets
    ...
    //    shift_comment(c);               // add comment to shifts with mutiply/divide  - if (curinst.opcix < 10 && ops->imd)
         ...
         switch (*tx)   // oct 1-12 used (0x1 0xc), oct  20 for sign, (0x10) oct 30 prob OK (0x18) oct 40 0x20 is SPACE, then oct 200, 0x80..
          {
           ...
           case 0x19:           // \31, \32, \33 octal  = skip operand if zero
           case 0x1A:
           case 0x1B:
             p_opskp(c, &tx);
             break;
           ...
           case 0x11:           // \21, \22, \23 octal  = print size & sign
           case 0x12:
           case 0x13:
     ...
     //        break;
           ...
           case 10:
             prt_jswp(c, &tx);     // bit and flag jumps   JB, JNB
             break;
