...
/********** COMMAND Option letters -
 ...
 D                      
 E                      ENCODED ADDRESS - type, rbase | offset
 F                      special function - type, addr, cols sign, etc
 G    Auto sub names
 H    set 8065
 I
 J
...
{"byte"    , set_data, pp_wdbl,   2, 2, 1, 1, 0, 2, 1, 1, "%[:PSX]%n" },           // group 1,  mergeable, lowest prio
{"word"    , set_data, pp_wdbl,   2, 2, 1, 1, 0, 2, 1, 1, "%[:PSX]%n" },
{"long"    , set_data, pp_wdbl,   2, 2, 1, 1, 0, 2, 1, 1, "%[:PSX]%n" },
...
{"opts"    , set_opts, pp_dmy,    2, 0, 0, 0, 0, 0, 0, 0, "%[:CDHLGMNPS]%n" },
...
AUTON anames [] = {
 ...
 {1, G, 1, 1, 1, "UUBFunLU" },         // 5   auto subroutine names for function and table lookup procs
 {1, G, 1, 1, 5, "USBFunLU" },  
 {1, G, 1, 5, 1, "SUBFunLU" },  
 {1, G, 1, 5, 5, "SSBFunLU" },  
 
 {1, G, 1, 2, 2, "UUWFunLU" },         // 9
 {1, G, 1, 2, 6, "USWFunLU" },  
 {1, G, 1, 6, 2, "SUWFunLU" },  
 {1, G, 1, 6, 6, "SSWFunLU" },  
...
uint casg[8] = {0,0,0,0,0,0x80,0x8000,0x8000000};                   // sign bit mask
...
LBK* add_data_cmd(uint start, uint end, uint com, uint from)
 {
  ...
  if (nobank(start) < (PCORG+0xc)) return NULL;
  if (icaddrse(&start,&end))       return NULL;
   ...
   if (com & 32) b->cmd = 1;
   #ifdef XDBGX
   DBGPRTN("attempt add data %x-%x %s",start,end, dirs[b->fcom].com);
  #endif

   z = inscmd(&datch,b);
   #ifdef XDBGX
   if (z == b)   DBGPBK (b,"add datcmd");
   else   DBGPBK (b,"ign datcmd");
           ...
           if ((r->ssize & 3) > 1 && a->cnt > 1)
             {
              ...
              #ifdef XDBGX
              DBGPRTN("Change size to %d", r->ssize);   
              #endif
             }
          }
        }
       ...
       if (ofst && opl->sigix == 17)
         {                // CALL,
          ...
          if (x && !x->emulreqd)
            {
                #ifdef XDBGX
             DBGPRTN("SET EMURQD for sub %x", x->start);
             #endif
   ...
   for (i=STKSZ-1; i >= 0; i--)
      {
        ...
        if (t->type == 1 && t->reg == o->addr && t->popped)
          { // found
            ...
            r->ssize = 2;                     // incs are ALWAYS word          
...
void add_args(LBK *l, int dreg, int args)
{
  ...
  size = args - dtsize(l->adnl);          // new args required
  reg = dreg;                              
  ...
  while (size > 0)
   {  
     ...
     size = args - dtsize(l->adnl);     
   }

   l->end  = l->start+args-1;               // set size correctly 
   l->size = dtsize(l->adnl);               // needs to be multiple ?
...
void upd_watch(SBK *s, INST *c)
 ...
 // RST *r;
  ...
  if (s->scanning && !get_rbinv(o->addr))
   {        // rbase checks only in scanning mode
    ...
    if (c->wop)
     {
      ...
      if (!b)
       {        // only add an RBASE if ldw or stw and immediate and not zero and not invalid
                // NO !!   A9L does an ad3w at 7096 for 11,12,13 - and why 11 ? (divide)

        if(c->opcsub == 1 && opctbl[c->opcix].sigix >= 11 && opctbl[c->opcix].sigix <= 13 && o->val)
             b = add_rbase(o->addr, val,0,0);
       }
      else 
       {        // already exists, invalidate if not same value or not immediate
         if (!b->cmd && (b->val != val || c->opcsub != 1))
  ...
  if (s->scanning || s->emulating) 
    {
       ...
       { 
        ...
        if (anlpass < ANLPRT)
          {
           if (s->scanning) DBGPRT( "%05x RAM %x = %x (%d)",s->curaddr,  o->addr, o->val, o->wsize);
           if (s->emulating) DBGPRT( "%05x ERAM %x = %x (%d)",s->curaddr,  o->addr, o->val, o->wsize);
...
// FIXED OFFSET ?????    if (s->foff) prt("D %x ", s->addr);
...
// all of these at present are whole read symbols with default size (e.g. subr)
  ...
  // verify address via vtype here;
  // verify type 1 = subr, 2 = code, 3 = data

  xsym = get_sym(0, addr,0);          // read and anywhere

  if (!xsym) 
    {
     xsym = add_sym(0, NULL, addr,0,0);     // add new empty (read) symbol
     if (xsym)  xsym->xnum = nix;                    // set autonumbered
    }
  else
    {
     if (!xsym->cmd && xsym->xnum && nix > xsym->xnum) xsym->xnum = nix;
    }
  return xsym;
}
          ...
          if (!t->inv && t->start < s->nextaddr)                     // s->end
            {
             ...
             DBGPRTN("INV scan overlapped by next %x-%x by %x-%x",s->start, s->nextaddr,t->start, t->nextaddr);
             #endif
...
SBK *add_escan (uint add, SBK *caller)
{
 ...
 //     get_scanpars(s);                    // ?? ????? // get new pars (and cksum)
      s->caller = caller;
      s->substart =  s->start;               // new subr
    } 
  ...
  switch (a->enc)
   {
    ...
    case 8:      // direct offset ?
     val += a->data;
     //val = nobank(val); not if data is proper address ?
     return val;
    break;
 ...
 */
  ...
  #ifdef XDBGX
  DBGPRTN("Check Func %x, IS%x, OS%x", xofst, nm->signin, nm->signout);
  #endif 
  ...
  endval = casg[nm->signin];                   // end value
  startval = (~endval) & camk[nm->signin];     // start value
   ...
   DBGPRTN("Func End at %x", xend);
    ...
    if (k && !k->cmd && !k->adnl)
      {                                     // cmd OK, add size etc (2 sets for func)
       ...
       a->cnt = 1;
       a->ssize = nm->signin;                         // size, word or byte
       a->dcm = 1;                              // default to decimal print

       a->next->cnt = 1;
       a->next->ssize = nm->signout;                  // size out
       a->next->dcm = 1;                        // default to decimal
      }
   //return ans;
   
   add_ovr(ovrreg, ovradd, ans & 0xff);
   if (nm->signout & 2) add_ovr(ovrreg+1, ovradd, ans >> 8);
}
...
void fnplu (SIG *s,  SBK *blk) 
 {
   ...
   uint i;
   int size;
   uint imask, omask, isign, osign;

   if (!s || !blk) return;
   if (!blk->scanning) return;
   if (!s->v[3]) return ;
   if (get_subr(blk->substart))  return;
...
//   if (s->v[3] && !get_subr(blk->substart))      // return if not true
...
#ifdef XDBGX
     DBGPRTN("add fsub %x", blk->substart);
     #endif
     subr = add_subr(blk->substart);
     subr->sig = s;
 ...
 DBGPRTN("set nodata %x", t->start);
     ...
     osign = (s->v[10] & 0x1000) ? 4 : 0;              // signed out if bit SET, 1000 = signed if NOT set 
     isign = (s->v[11] & 0x1000) ? 4 : 0;              // signed in  if bit SET, 1000 = signed if NOT set 
     osign |= size;
     isign |= size;
     ...
     if (find_pn_opcodes (1, blk->substart, 20, 1, 9))     // look for a OR (20 instructions) from front of subr
         ...
         if (sinst.opr[2].addr == s->v[3])
          {
           ...
           #endif
           if (omask & sinst.opr[1].val)  osign ^= 4;    // flip sign out if match;
           if (imask & sinst.opr[1].val)  isign ^= 4;    // flip sign in  if match
          }
         ...
         if (find_pn_opcodes (1, sinst.ofst+3, 20, 1, 9))       // look for another OR (forwards of first one)
          {
           if (sinst.opr[2].addr == s->v[3])
            { 
              ...
              #endif
              if (omask & sinst.opr[1].val)  osign ^= 4;
              if (imask & sinst.opr[1].val)  isign ^= 4;
            }
          }
        ...
        for (i = 5; i < 13; i++)
          {  
           if (anames[i].signin == isign && anames[i].signout == osign)
             {
              new_nsym(i,blk->substart);             // add subr name acc. to type
              subr->spf = i;
              #ifdef XDBGX
              DBGPRTN("func xnum=%d (%x)", i, blk->substart);
              #endif
              break;
             }    
          }      
...
// probably need a maxend safety here..... and a sign check....
  ...
  #ifdef XDBGX
   DBGPRTN("SET TAB Add %x cols %x Rows %x inx %x ss %d", ofst, cols, rows, ainx,anames[ainx].signout);
   #endif
...
// mark end of subr ??
     ...
     case J_SUB:
        ...
        scan_subbs(newsc, c);
         ...
         {
             #ifdef XDBGX
             DBGPRTN(" ARGS FOUND %x (s%x) set emulrqd for %x (s%x)", newsc->start, newsc->substart, caller->start, caller->substart);
             #endif
         ...
         if (newsc && newsc->emulreqd)     // && !caller->emulating)
         {
 ...
 /* if (!size)
    {
     ...
     not used with variable args working
     xsub = get_subr(c->opr[1].addr);         // t->addr;
  
     if (xsub)
      {
       size = xsub->nargs1;
       if (PSSRC && xsub->nargs1) pp_lev(ofst,get_cadt(xsub),0,xsub->nargs1);     // do params at min width, no trail comma
      }  
    }   */
...
// do args from WOP of operand

// doesn't work for indirects............??
   ...
   for (i = 1; i <= c->numops; i++) 
    {
     o = c->opr+i;
     r = find_rgstat(o->addr,0);          // how to get 
     if (r && r->arg)
     {
       if ((r->ssize & 3) < (o->ssize & 3)) r->ssize = o->ssize;                         // just do size
           #ifdef XDBGX
...
*/
    ...
    if (mask & 2)
        {           // carry
         c->psw &= 0x3d;          // clear CY
         if (val > camk[size]) c->psw |= 0x2;      // set CY  
        }
      if (mask & 4)
        {           // OVT - don't clear, just set it.
         if (val > camk[size]) c->psw |= 0x4;      // set OVT  
        }

      if (mask & 8)
        {           // OVF
         c->psw &= 0x37;          // clear OVF
         if (val > camk[size]) c->psw |= 0x8;      // set OVF  
        }
...
void popw(SBK *s, INST *c)
{
   ...
   if (!s->caller || !o->addr) return;            // ignore if no caller   
      ...
      {           // scan UP callers
       ...
       if (!t->popped)
         {   // not popped yet(or pushed back)
          ...
          r = add_rgstat(o->addr);          // find or add (was origadd)
      ...
      if (inx >=0)
        {
         ...
         org = add_rgstat(o->addr);            // find or add
         t->reg = o->addr;
  ...
  if (s->emulating)
    {
      ...
      if (sce) org = find_rgstat(sce->addr, 0);      //t->origadd);         //nearby OK 
      ...
      if (org)
        {     // found source reg
          if (org->arg)
              // reg is flagged as arg, so use size
           {
            // really what we are interested in for now is ONLY the SIZE
              ...
              if ((o->ssize & 3) > (org->ssize & 3))
              {
                org->ssize = o->ssize;
                #ifdef XDBGX 
               DBGPRTN("SIZE ARGX %x=%x (%d) at %x", o->addr, org->reg, org->ssize, s->start);
               #endif
              }
              // check if word op......
        
                org = find_rgstat(org->sreg, 0);
                if (org)
                {
                 if ((o->ssize & 3) > (org->ssize & 3)) {org->ssize = o->ssize;
                  #ifdef XDBGX 
                 DBGPRTN("SIZE ARGX POP %x (%d)", org->reg, org->ssize);
                 #endif
                 }
                }
           }  
         ...
         if (org->popped)
           ...
           {
            nrg = add_rgstat(o->addr);
            if (nrg)
             {
              nrg->arg = 1;
              if ((o->ssize & 3) > (nrg->ssize & 3)) nrg->ssize = o->ssize;
...
int readpunc(uint posn, uint end)
{
     ...
     switch(optl)
          {
           ...
           case 'D' :
             ...
             cmnd->posn+=rlen;
             c->enc = 8;
             c->data  = v[0];    // Word + fixed address offset (funcs) - bank swop vect
             break;             
...
// if (ans)  flbuf[end] = '\0';
  ...
  if (crdir->maxpars)       // read up to 4 following addresses into p (if any)
    {
     ans = getp(cmnd.p, &cmnd.posn);

     if (ans < 1) return do_error("Parameters Required",cmnd.posn);
     if (ans > crdir->maxpars) return do_error("Too many Parameters",cmnd.posn);
     cmnd.npars = ans;
  ...
  if (crdir->name_ex && cmnd.posn < maxlen &&  flbuf[cmnd.posn] == '\"')
   {
...
void prt_subs (void)
{
  ...
  wnprtn("# ------------ Subroutine list (special funcs only)");
...
short init_structs(void)
 ...
 // memset(&wramlist,0, sizeof(PARS));
  //zeropars = get_scanpars();               // a 'zero' list
...
void do_branch(uint addr, int cnt, uint rg1, uint rg2, uint rg3, int ans1, int ans2, int ans3)
 {
   ...
   while (xcnt) 
      ...
      if (!xcnt && ans1 && ans2 > 1)
       {
        set_str(ans1, anix, ans2, 0);      // add tab with one row
       }
 ...
 while(ix >= 0 && ix < jptch.num)
   {  
    ...
    if (j->jtype == J_SUB)              // sub only for from FIRST call (why?) for call back to sub......
        ...
        //j->from+j->size to check for arguments............just finding is OK for funcs...but rg1 etc give regs to find
        ofst = j->from+j->size;
        g = find_data_cmd(ofst, C_ARGS,0);
        
        if (g)
          {
           //DBGPRTN("find args at %x for %x", g->start, rg1);    // assume first parameter ???
           
      //     x = find_ovr(ofst,rg1, 2, &ans1 );     // ans is still wrong ??  if change, then can get args themselves....
      //     if (x) ans1 |= datbnk;
