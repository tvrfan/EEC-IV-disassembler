...
//#define NUMPARS   12                 // num bytes in PARS block
#define STKSZ     10                  // size of fake stack(s)  
#define EMUGSZ    5                  // size of emu arguments list  
...
float  fdat;          // full float for divisor
...
uint ssize  : 3 ;     // (Z) 1,2,3 unsigned, 4,5,6 signed - for byte,word,long
// S,W,L,Y  replacement. if ssize = 0, don't print entry. 
...
//uint noprt  : 1 ;     // suppress print if this level (used for vects ) 

uint enc    : 4 ;     // (E) encoded data type (E) 1-15 (addr & 0xff) 8 is plain offset from full addr
...
// CADT *adnl;              // special functions, arguments, etc.
 struct xsig  *sig;               // if sig attached
...
{
  CADT  *adnl;
  uint   posn;             // char posn (for errors)

  uint  p[4];             // 4 params max
...
{                     // auto numbered symbol names
 ...
 int vtype   : 3;     // verification type for address 1 = subr, 2 = code, 3 = data
 uint signin  : 3;
 uint signout : 3;     // sign flags for func and tab lookup subrs
 const char *pname;
 }  AUTON;
