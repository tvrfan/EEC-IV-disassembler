...
// options (as bitmask letters in an int) used by sign as well as core

#define PSSRC     (cmdopts & 1)      // C source code
#define PFUNC     (cmdopts & 2)      // F auto function and table names
...
#define PSIG      (cmdopts & 0x100)      // Sauto Lookup Signature subroutine detect
...
// others here ? or a pointer ?? pointer is 8 bytes, uint is 32.....
// so got  12 bits ?? possible........
...
typedef struct          // operand storage, max 4 for each instruction
{
 ...
 uint astr    : 1;      // asterisk (for address of)
} OPS;
