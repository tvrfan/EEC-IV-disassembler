...
{ 3 ,  51,  1, 2,2, 2,1,6,0, S         ,shr ,"asrw"  ,  "\x2 >>= \x1;" },

{ 3 ,  47,  1, 2,2, 3,1,3,0, S         ,shr ,"shrdw" ,  "(long) \x2 >>= \x1;" },            // 11
{ 3 ,  39,  1, 2,2, 3,1,3,0, S         ,shl ,"shldw" ,  "(long) \x2 <<= \x1;" },
{ 3 ,  47,  1, 2,2, 7,1,7,0, S         ,shr ,"asrdw" ,  "(long) \x2 >>= (int)\x1;" },
...
{ 3 ,  51,  1, 2,2, 5,1,5,0, S         ,shr ,"asrb"  ,  "\x2 >>= \x1" },
...
{ 19,  75,  0, 1,0, 0,2,0,0, J|C|Y     ,0   ,"jnc"   ,  "\x8\x7\x30 < \x5) \x9" },
...
{ 19,  78,  0, 1,0, 0,2,0,0, J|C       ,0   ,"jge"   ,  "\x8\x6 >= \x5) \x9" },
 ...
 19,   66,  0,  1,0, 0,2,0,0, J|C|Y     ,0   ,"jc"    ,  "\x8\x7\x31 >= \x5) \x9" ,
 ...
 8 ,  0,   1,  3,3, 7,6,6,2, M         ,mlx ,"sml3w"  , "(long)\x3 = \x2 * \x1;",      // 100
 8 ,  0,   1,  3,3, 6,5,5,2, M         ,mlx ,"sml3b"  , "(int) \x3 = \x2 * \x1;" ,
 8 ,  0,   1,  2,2, 7,6,6,0, M         ,mlx ,"sml2w"  , "(long)\x2 = \x2 * \x1;",
 8 ,  0,   1,  2,2, 6,5,5,0, M         ,mlx ,"sml2b"  , "(int) /x2 = \x2 * \x1;",
 11,  0,   1,  2,2, 7,6,7,0, M         ,dvx ,"sdivw"  , "/x2 = (long)\x2 / \x1;",
 11,  0,   1,  2,2, 6,5,6,0, M         ,dvx ,"sdivb"  , "/x2 = (int)\x2 / \x1;",
   ...
   if (!z->l1d)
    {
     ...
     if (csub->addr != s->ofst)
      { 
        ...
        p = (SIG*) find(&sigch,p);     // find sign sig
    ...
    if (csub->addr != s->ofst)
     {
      ...
      p = (SIG*) find(&sigch,p);          // find sign sig ? but could be scaled sig
      if (!p) p = scan_msigs (csub->addr);
...
short find_fjump (int ofst)
{
  ...
  j = (JLT*) find(&jpfch,j);         // jpftab ofst.........from
  ...
  while (cmntofst < ofst)
    {
     c = cmtt;
     if (!c) break;                              // safety
     if (*c != '|' ) p_pad(cposn[3]);            // '|' = newline at front, no pad
     while (*c)
      {
       switch (*c)
        {
...
// need more checks in here.....opt & E  must be a stop , and probably CALL.
   ...
   //   if (x && x->opl->opt & E)  x = 0;                      // end instruction
      if (x && (x->opl->opt & (J|X)) == (J|X))  x = 0;        // call
...
//           case 4:  not used ???
	       ...
	       case 5:
            ...
            if (lwop > 0) p_opsc(&dmyinst,1);            // zero if not a compare
...
short init_structs(void)
 {
  ...
  memset(&dmyinst,0,sizeof(INST));   //clear dummy
  dmyinst.opr[1].imd = 1;
  dmyinst.opr[2].imd = 1;
  dmyinst.opr[3].imd = 1;                                  // print zero
