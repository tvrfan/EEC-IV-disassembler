...
CHAIN *chlist[] = { &jpfch, &jptch , &symch,  &basech, &sigch,
                    &codch, &scanch, &emulch, &subch , &sbcnch, &sgapch, &datch, &rgstch, &rgovch, &mblk};

#ifdef XDBGX
// names for chains for debug
   const char *chntxt[] = {"jumpf", "jumpt", "rsym ", "wsym ", "base ", "sign ",
                           "code ", "scan ", "emuls", "subscan ", "gaps", "subrs ", "data ", "regst", "regovr", "monb "};   // debug names for chains
#endif
 ...
 * 
#define PSSRC     (cmdopts & C)      // +sce       source code
#define PFUNC     (cmdopts & F)      // +tfnames    auto function and table names
#define PSTRS     (cmdopts & G)      // +ssubnames   auto subroutine names for function and table lookups
#define P8065     (cmdopts & H)      // +8065       8065 codeset (can be set internally)
#define PLABEL    (cmdopts & L)      // +labelnames labnames   auto label names (lab)
#define PMANL     (cmdopts & M)      // +manual     full manual mode

#define PINTF     (cmdopts & N)      // +intrnames   auto intrp func naming
#define PPROC     (cmdopts & P)      // +subnames   auto proc names (default subnn)
 ...
 * 
 * global to set all OR AND as separate flags ? builtin_popcount.
 * +tab
 * +func
 * 
 * "clear", "default",  "table",  "function",    "sce", "tfnames", "ssubnames", "8065",  "labelnames",  "manual",  "intrnames", "subnames"
 * 
 * 
...
const char *opts[11] = {"default", "table", "function", "sce", "tfnames", "ssubnames", "8065",  "labelnames",  "manual",  "intrnames", "subnames"}; 
...
{ 22,  0, 0,   1, 2,1, 1,1,3,0, nrm  ,"norm"  ,  "\21 = normalize(\22)" },
...
{       // debug print cmd blk
  ...
  DBGPRT(" %s %05x %05x ",  cmds[b->fcom], b->start, b->end);
  va_end (args);
}
...
void freechain(int ix)
{
  ...
  x = chlist[ix];
    ...
    DBGPRTN("max=%5d tot=%5d allocs=%2d esize=%3d size=%6d   %s", x->DBGmax, x->pnum, x->DBGallocs, x->esize, size, chntxt[ix]);
  #endif
...
//  #endif
  ...
  if (nobank(start) < (PCORG+2)) return NULL;     //+0xc)) return NULL;
  if (icaddrse(&start,&end)) return NULL;         // this is CODE validator
...
for (i = 0; i < BMAX; i++)
 {
  ...
  if (x->bok)
    {
     wnprt("# bank  %d %5x %5x %5x", i, x->filstrt, x->filend, cmdaddr(x->maxpc));
     #ifdef XDBGX  
...
SYT *add_sym (int rw, const char *fnam, int add, int bitno, int start, int end)
 {
   ...
   if (ans)
   {
     ...
     if (rw & C_CMD) s->cmd = 1;                  // by user command
      ...
      if (bitno) DBGPRT (" + BIT %d", (s->addb & 0xf)-1);
...
void DBG_sbk(const char *t, SBK *s, int p)
{
 ...
 DBGPRT(" scnt %d type %d", s->scnt, s->type);
...
// for tables, horiz, vertical,diagonal ?
...
// minimum possibly IS best - lowest possible absolute difference between rows.
// can handle signed by adding 0x100 to lower value ??  may be - along with bit mask
// and counting.......
        ...
        if (s->substart == caller->substart)
         {
          ...
          if (sbcnch.num != num) 
            {
             #ifdef XDBGX
             DBGPRTN("NEWCH %d %d", sbcnch.num, num);
             #endif
   ...
   for(i = start; i <= end; i+=2)       // always word addresses  (score > 0?)
     {
      ...
      if (l)
        {
         #ifdef XDBGX
           DBGPRTN("hit code %x %s",i,cmds[l->fcom]);
         #endif 
         end = i-1;
         break;
        }
...
// add a SIZE check
   ...
   {                // is a minimum of 8 pointers
     #ifdef XDBGX
     DBGPRT ("REJECT LIST %x from %x, %d ptrs",start, c->ofst, (end-start)/2);
     DBGPRTN(0);
     #endif
     return 0;
   } 
...
void fix_sym_names(const char **tx, INST *c)
{
  ...
  if (opl->nops > 2)
    ...
    if (c->opr[2].addr == 0)         // x+0,   drop 0
     {
      c->opr[2] = c->opr[3];                // drop op [2]
      if (tx) *tx = opctbl[LDWIX].sce;        // use ldx string TEMP  ([2] = [1] )
     } 

    if (c->opr[1].addr == 0)      // 0+x so drop 0
     {
      c->opr[1] = c->opr[2];
     ...
     if (d->fcom > C_DFLT && d->fcom < C_LONG)
      {
        ...
        if (s->start <= d->start && s->nextaddr >= d->start )
          {
              #ifdef XDBGX
            DBGPRTN("DATA OVERLAP 1 %x %x %s-> %x %x %s (from %x, %d)", s->start, s->nextaddr, "SCAN", d->start, d->end, cmds[d->fcom],d->from, d->opcsub);
            // just assume it's dodgy...........for now
            DBGPRTN("delete %x-%x", d->start, d->end);  
            #endif
            chdelete(&datch,i,1); i--;  
          } 
       ...
       if (ix > 0)
         {
          ...
          if (p->start <= d->start && p->nextaddr >= d->start )
           {
               #ifdef XDBGX
            DBGPRTN("DATA OVERLAP 2 %x %x %s-> %x %x %s (from %x, %d)", p->start, p->nextaddr, "SCAN", d->start, d->end, cmds[d->fcom], d->from, d->opcsub);
            #endif
            if (d->opcsub == 3)
              ...
              if( j >= 0 && k <= 16)
               {
                ...
                d->end += j;
                   #ifdef XDBGX
                DBGPRTN("move data to %x-%x", d->start, d->end);
                  #endif
               }  
              else
               {
...
void scan_sgap(uint addr)
 {
  ...
  {
    #ifdef XDBGX
     DBGPRTN("Ignore gscan for %x, fails rpt check", addr);
    #endif
    return;
  }
      ...
      //      {
           ...
           //     }
                ...
                if (n->start < zz)  zz = n->start;
                 ...
                 #ifdef XDBGX  
                DBGPRTN("******************************************* Data GAP s %x-%x", d->start, zz-1); // not if found something before it.
                #endif
...
{  // NOT found scan block
    ...
    //  #ifdef XDBGX  
     //           DBGPRTN("******************************************* Data GAP d %x-%x", d->start, n->start-1); // not if found something before it.
     //           #endif
}
         ...
         if (d->start > n->start)
            {
             // code gap with nothing else in it.
             #ifdef XDBGX  
             DBGPRTN("******************************************* Code GAP %x-%x", s->nextaddr+1, n->start-1); // not if found something before it.
             #endif
             scan_sgap(s->nextaddr+1);
            }
 ...
 for (ix = 0; ix < z->num; ix++)
    {
     ...
     if (c->fcom == C_TIMR) wnprt ("# "); 
     wnprt("%-7s %x %x", cmds[c->fcom], cmdaddr(c->start),cmdaddr(c->end));
