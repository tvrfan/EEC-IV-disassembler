...
CHAIN scanch = {0,0,750,sizeof(SBK),   0,0,0,fscn ,cpscan , cpscan};      // scan
...
{"code"    , set_code, pp_code,  0,   2, 1, 0, 0,31, 1, 0  } ,                    // overrides everything
...
#define LDWIX  41          // for 3 op replacements to "x = y" - just like LDW
#define JMPIX  75          // for plain 'goto' jump output (conditionals)
...
/*
 ...
 12 load      13 store  14 push     15 pop       16 sjmp    17 subr
...
{ 8 ,  1, 0,   1, 3,3, 2,1,1,1, mlx   ,"ml3b"  , "\23 = \2 * \1"  },   // 18
{ 8 ,  0, 0,   1, 3,3, 6,5,5,2, mlx   ,"sml3b" , "\23 = \2 * \1" },    // 19
{ 8 ,  1, 0,   1, 3,3, 3,2,2,2, mlx   ,"ml3w"  , "\23 = \2 * \1"  },   // 20
{ 8 ,  0, 0,   1, 3,3, 7,6,6,2, mlx   ,"sml3w" , "\23 = \2 * \1" }, 
...
{ 18,  0, 0,   0, 2,2, 2,2,1,0, djm   ,"djnz"  , "\2--;\xbif (\2 != 0) \x9" },  //72
...
int aucmtofst;              // auto comment ofst (where to put it)
...
int ramlist[NPARS];          // (16) last updated ram address list for HOLD (address & size)
int fstack[32];              // fake (and lazy) stack for parameter tracking
int fsptr;
...
void paddr(int addr)
{
  // printer for address with or without bank. (drop trailing space)
...
// address handling 0xfffff  (4 bits bank, 16 bits in bank)
...
uchar* map_addr(int addr)
 {
  ...
  if (nobank(addr) < PCORG) return bkmap[BMAX].fbuf;            //RAM or regs
...
// extra checking for LBK commands

int olchk(CHAIN *x, int i, LBK *b)
 {
...
// now check for overlaps
   ...
   if (b->start >= t->start && b->end <= t->end)  {ans |= L; ans |= C;}  // t contains b
   else
   if (b->start <= t->start && b->end >= t->end)  {ans |= L; ans |= S;}  // b contains t
   else
    {  
     if (t->start >= b->start && t->start <= b->end) {ans |= L; ans |= F;}       // overlap (start)
     if (t->end   >= b->start && t->end   <= b->end) {ans |= L; ans |= R;}       //overlap (end)
    }
   if (ans & L)
    {
     if (!t->cmd && (dirs[b->fcom].ovrde & (1 << t->fcom))) ans |= W;   // b can overwrite t
     if (!b->cmd && (dirs[t->fcom].ovrde & (1 << b->fcom))) ans |= V;   // t can overwrite b
    }
...
void merge(LBK *t, LBK *s)
 { // merge two blocks, t = s update BOTH but only t is kept
...
int split(LBK *t, LBK *s, int cf)
{
  // t is chained block, s is candidate block
  // cf, chkflags says what to do
  // W means s should overwrite t
  ...
  if (cf & W)
    { 
     ...
     if (cf & F)
       {
         // front overlaps - move t up and allow insert
         t->start = s->end+1;
         return 1;
       } 
    } 
  if (s->start >= t->start && s->start <= t->end)
    { // s->start is within m range
      if (s->end > t->end)
      s->start = t->end+1;
      return 1;
    } 
  if (s->end  >= t->start && s->end   <= t->end)
    {
     s->end   = t->start-1;
...
int inschk(CHAIN *x, int *ix, LBK *b)
{
  ...
  max = *ix;            //block returned from find (compare)
  chkn = 1;
  ...
  while (chkn)
    {
     ...
     if (chkn & L)
      ...
      // overlaps with next block
       if (chkn & C)
         {
         // b already contained in l - do nothing - what if W or V ?
          #ifdef XDBGX  
            DBGPBKN(t,"contained in");
          #endif
          ans = 0;          // no insert
         }
       else
       if (chkn & M)
        ...
        if (chkn & W)
        {
         // overwrite allowed     WRONG if contained !! NOT if S!!
         #ifdef XDBGX 
           DBGPBKN(t,"Split with");
          #endif
         ans = split(t,b,chkn);
...
void upd_ramlist(int addr, int size)
 {
  ...
  // size 2 if 0x8000
  // [0] = always newest entry

  if (addr <= minwreg || addr >= PCORG) return;     // only keep from general regs to PCORG
  size &=3;                                         // no longs (yet)  
  j = NPARS-1;                                      // move 15 entries up if not found (kill oldest)
  ...
  for (i = 0; i < NPARS-1; i++)
    ...
    memmove (ramlist+1, ramlist, j * sizeof(int));
    ramlist[0] = addr;
    if (size & 2) ramlist[0] |= 0x8000;       // flag 0x8000 if size is 2 or 6 (signed)
 }
...
// check for autoinc (can be on the read register....always [1])
 ...
 if (curinst->inr)
  {    // put original back for increment, but keep indirect address in [0]
   w = o->addr;                 // address accessed (not register)
   o->addr = curops->addr;      // restore orig register
   o->val = curops->val;        // and orig value (for possible increment)
   curops->val = w;             // swop final (indirect) address to [0].val;
  }

if (curinst->inc)    
    {     // auto increment set - update value, check for subr params
      o->val += casz[o->ssize];       // autoinc is by op size

      // may not need the upd_ramlist ??     
      if (upd_ram(o->addr,o->val,2))  upd_ramlist(o->addr,2);
      #ifdef XDBGX
      if (anlpass < ANLPRT) DBGPRTN( "%05x RAM++ %x = %x (%d)",s->curaddr,  o->addr, o->val, 2);
      #endif
    } 
}
...
int outside (JLT* b, JLT *tst)
 {
   ...
   if (tst->jtype != S_SJMP && tst->jtype != S_CJMP) return 0;      // ignore subr and return jumps
...
for (ix = 0; ix < jpfch.num-1; ix++)
    {
     ...
     if (!f->back && !f->uci && !f->uco && !f->retn && f->jtype == S_CJMP) f->cbkt = 1;      // conditional, forwards, clean.
    }
...
void DBG_sbk(SBK *s)
{
 ...
 if (s->sub)  DBGPRT (" sub %05x", s->sub->start);   else DBGPRT("         ");
 if (s->caller)
 { 
  DBGPRT (" callst %05x r %05x", s->caller->start, s->caller->retaddr);
  if (s->caller->sub) DBGPRT (" csub %05x", s->caller->sub->start);
 }
 if (s->cmd) DBGPRT (" cmd");
 if (s->inv) DBGPRT (" INV");
     ...
     if (!sub->cmd && !sub->cname && sub->xnam)
       { // not cmd, name change allowed....
        ...
        {  // check if sym exists alreaady....
          add_nsym(sub->xnam,sub->start);             // add subr name acc. to type
        }
      }    
    }
   #ifdef XDBGX      
 ...
 while(x)
   {
    ...
    #ifdef XDBGX
    if (c > s->nargs1) DBGPRT("possible incorrect size %d - %d ", c, s->nargs1);
    DBGPRTN("set size = %d for %x",s->nargs1, s->start);
...
CSIG* init_csig(SIG *x, SBK *b)
{
   ...
   #ifdef XDBGX  
   DBGPRT (" Sub %x init csig ", b->sub->start);
   //   DBG_CSIG(c);
   //  DBGPRTN(0);  
   #endif  
 return c;
...
void process_sig(SIG *x, SBK *b) 
 {
  ...
  if (b->sub && b->sub->cmd && !x->ptn->spf)
     {
       #ifdef XDBGX  
       DBGPRTN("Ignore sig %s subr %x cmd",x->ptn->name, b->sub->start);
       #endif
   ...
   // correct that sigs are chained at the back (i.e. in order),
     ...
     if (!c)  
         {                          // not a duplicate....
          ...
          #ifdef XDBGX
           DBGPRT ("(PRCS) ");
           DBGPRTN ("%x (PRC) ADD csig to sub %x ",b->curaddr, b->sub->start);
          #endif
         }
    }
  ...
  for (ix = 0; ix < scanch.num; ix++)
   {
    ...
    if (!s->inv)
      ...
      {
        #ifdef XDBGX
        DBGPRTN("INV scan %x - %x",s->start, s->end,0);
        #endif
      }
  ...
  while (x)
   {
     ...
     if (s)
      { 
          ...
          if (blk->sub) DBGPRT(" sub %x", blk->sub->start);
...
void end_scan (SBK *s, int end)
{
  ...
  if (end) 
   { 
	if (end > maxadd(end)) end = maxadd(end);            // safety check
	s->end = end;
   }

  if (!s->held && s->end >= s->start) s->scanned = 1;
...
//      DBG_scanpars(s);
   ...
   chinsert(&scanch, ix, s);            // do insert
    ...
    if (caller) 
      {
       if (caller->sub) DBGPRT(" +sub %x", caller->sub->start);
       DBGPRT (" from %x", s->retaddr);
      }
   DBGPRTN(0);
  #endif
...
void chg_subpars (SXT* sub, int reg, int size)
{
 ...
 #ifdef XDBGX 
   DBGPRT("In chg_subpars sub %x  p%x to %d", sub->start,reg, size);
 #endif
...
int get_subpars(SXT* sub, SBK* blk, int pno)
{
 ...
 addr = blk->nextaddr;
  ...
  #ifdef XDBGX 
  DBGPRT("In get_subpars sub %x at %x", sub->start,addr);
  #endif
 reg = 0;
...
if (z->disb) return 0;                // DON'T copy disabled csigs.
  #ifdef XDBGX
  DBGPRTN("Cpy csig lvs [%s %d %d] to %x ",z->sig->ptn->name, z->lev1, z->lev2,cblk->sub->start); 
  #endif
...
void fnplu (CSIG *z, SBK* nsc, SBK *blk)
 {
  ...
  if (size > 1 && (adr & 1))
   ...
   adr |= datbank;       // need this as get_reg only does 16 bit
     ...
     {
       #ifdef XDBGX
       DBGPRTN("FUNC Invalid addr %x from %x", adr, blk->curaddr );
       #endif
      return;
     }
...

{
  ...
  DBGPRT("In TBPLU ");
   ...
   if (!val_data_addr(adr))
      {
        #ifdef XDBGX  
        DBGPRTN("TABLE Invalid addr %x from %x", adr, blk->curaddr );
        #endif
        return;
      }
...
void fparp (CSIG *z, SBK *nsc, SBK *caller)
    ...
    * [3] param list (incremented) */
    ...
    if (!z->l1d)
    {
     ...
     #ifdef XDBGX
       DBGPRT("subr %x add cadt (%x)",csub->start,caller->curaddr);
       //PRT_CADT(d->cadt);
       DBGPRTN ("  fparp done");
     #endif  
    }
 ...
 //  int i,j;
        ...
        {                 // not processed yet - add CADT
          ...
          if (a)                                 // safety
           {                                     // add LOCAL param as a CADT    (this is number of pars to get from caller)
            ...
            set_size(csub);
            #ifdef XDBGX
              DBGPRTN("sub %x add cadt L=%d", csub->start, csub->nargs1);
              //PRT_CADT(d->cadt);
            #endif
   ...
   if (z->lev1 == 1)
    ...
    if (!z->l1d)
    {
     ...
     #ifdef XDBGX  
       DBGPRT("subr %x add cadt R (%x)",csub->start,blk->curaddr);
       //PRT_CADT(d->cadt);
       DBGPRTN(0);
     #endif  
...
void grnfp(CSIG *z, SBK *nsc, SBK *blk)
{
 ...
 // SIG *s;
  ...
  csub = nsc->sub;
  #ifdef XDBGX
  DBGPRTN("In GRNF subr %x (%d) for %x", csub->start, csub->nargs1, blk->curaddr );
  #endif
 ...
 if (b)                        // may be sub, but possibly not....
  {
    init_csig(x, b);
    #ifdef XDBGX   
     DBGPRTN ("%x Add csig to sub %x ",b->curaddr, b->sub->start); 
    #endif
  }
...
void do_substructs (SBK* nsc, SBK *caller)
 ...
 //if (sub->cmd) DBGPRTN("Sub CMD");
   #ifdef XDBGX
   DBGPRTN("at %x for sub %x", caller->curaddr, sub->start);
   #endif
 ...
 if (sub->sgclc && sub->sdat) freecadt(&sub->sdat->cadt);         //recalc set - clear ALL cadts so far
  ...
  while (z)                          // process each sig in order along chain
   ...
   if (args != sub->nargs1) fix_scans(sub->start);   // rescan blocks if params have changed.
...
int do_hold(SBK *newsc, SBK *caller)
 {
  ...
  get_scanpars(caller);                          // doesn't work without this...reqd for scanned (= reused) subr with new params
  ...
  switch (type)                                // requested scan type
    {
     ...
     case S_CJMP:                        // conditional jump
        ...
        */
        ...
        if (g_bank(addr) == g_bank(curinst->ofst) && addr < curinst->ofst)
         {
             if (get_opstart(addr)) break;    // backward jump probably a loop
         }
     ...
     case S_SJMP:                       // static jump
         ...
         */ 
         ...
         if (g_bank(addr) == g_bank(curinst->ofst) && addr < curinst->ofst)
         {
             if (get_opstart(addr)) break;    // backward jump probably a loop saves about 4%
         }
...
void op_val (int pc, int ixx)
  ...
  if (ixx < 0)
   ...
   {
    bank = opbank(datbank);   // data bank otherwise
    ix = ixx;
   } 
...
//  if (ixx < 0) o->ssize = 2;             // for jumps
  ...
  // indirect and indexed etc cases handled in do_code
}
...
{
   int addr, i, score, lowcadd, lastiadd, end;
   SBK *s, *t;
   LBK *l, *k;
   ...
   if (sz) end = start + sz;
   else   end = start + 0x100;               // 128 vectors max ? A9L is 0xa4
   score = 0;                     // max score
  ...
  // check start against curinst+cnt to see if an extra offset needs to be added ?
   ...
   #ifdef XDBGX
    DBGPRT("In check_vect %x sz=%x R=%x lc=%x", start, sz, retaddr, lowcadd);
    DBGPRTN("from %x", curinst->ofst);
   #endif
   ...
   for(i = start; i < end; i+=2)       // always word addresses  (score > 0?)
     {
      ...
      if (s)
        {
         #ifdef XDBGX
          DBGPRTN("hit scan %x t%d", i, s->ctype);
         #endif
         end = i-1;
         break;
        }
      ...
      if (retaddr)  addr |= g_bank(retaddr);    // bank from return addr
      else addr |= opbank(codbank);    //g_bank(start);    // bank from start addr (not sure if this is right)
      ...
      if (!val_jmp_addr(addr))
        {
         ...
         if (!lastiadd) lastiadd = i;                          // keep last one
           ...
           {
            end = lastiadd-1;                  // not allow 2 consecutive invalids 
            break;
           }
         addr = 0;                               // remove address
         //if (i == start) start +=2;            // allow first entry to be invalid ?
        }
    ...
    while (x && cnt < 16)
     {
      ...
      if (opl->sigix == 17)  break;              // call  (ans = -1)
      ...
      if (opl->pswc)                  // sets PSW, allow target
         {
           *z = x;                   // instance found
           ...
           if (ans && !x->opr[ans].addr) ans = -ans;  // wop is R0
           break;
         }
     }
  ...
  int i;
   ...
   if (ix >= 0x11 && ix <= 0x13)
      {                 // extra size/sign requested
       i = ix & 3;
       if (i == 1 && (x->inx || x->inr)) i = 0;     // not for indexes or indirect
       if (x->opr[i].sym) i = 0;
       if (!x->opr[i].rgf) i = 0;
  ...
  if (x->inx && ix == 1)
    {                                        // indexed ops are SIGNED offset
      ...
      if (p_sc (x, ix))
        {
    //     pchar(0x20);   
         pchar( (b->addr < 0) ? '-' : '+');      // if first op printed, with spaces
    //     pchar(0x20);   
        }
      p_sc (x, 0);
      if (o->bkt) pchar (']');
...
void auto_comment (int ofst)
{
// put auto generated in HERE
// aucmtofst;
    if (*autocmt) {  // && autcmtofst < ofst){
...
void fix_sym_names(const char **tx)
{
  ...
  if (opl->nops > 2)
    {
     ...
     if (curops[2].rbs && !curinst->inx)    // && !curinst->imd)        // RBASE by addition, not indexed
      {
       b = curops+1;
...
void do_size(int op, int rw)
 {
   if (curops[op].sym)  return;            // not if symbol name
   if (!curops[op].rgf) return;
   if (op == 1 && (curinst->inx || curinst->inr)) return;     // not for indexes or indirect

   if (curops[op].wsize && rw) op = curops[op].wsize; else op = curops[op].ssize;
...
int cmnt_op(INST *last)
{
 const char *s;
 char t;
 int ps;
   
 ps = sprintf(autocmt, "## jump if (");
...
// Replace if (CY) with various sources, depeneding upon whether cmp found
   ...
   lwop = find_last_psw(&last);             // get last psw changer (last is always valid)
   if (!last)  return;                      // valid one not found, or subr call etc. use default
...
// next ones don't really work in a maths sense....so embed a comment instead.
   ...
   if (opl->sigix == 6  || opl->sigix == 3 || opl->sigix == 25)
    ...
    //      if (opl->sigix == 24) ainx ^= 1;  // swop lowest bit for dec (as it's a borrow with subr) 
          cmntinst = last;
          aucmtofst = s->nextaddr;      //curaddr;
          ps = cmnt_op(last);
  
        if (!(ainx & 1))  ps += sprintf(autocmt+ps, " not");
        sprintf(autocmt + ps, " overflowed");
        }
    
     if (opl->sigix == 24)
      {       // dec
         cmntinst = last;
         ps = sprintf(autocmt, "## jump if \1");
         ps += sprintf(autocmt+ps, (ainx&1) ? " >= " : " < ");  // inx 39
...
void do_cond_ops(const char **tx, int ainx)
{
   ...
   if (lwop == 0)
     {       // compare found, style R2 == R1
      if (last->opr[2].addr == 0)     // && (curinst->opcix == 68 || curinst->opcix == 69))
       {  // jeq or jne with zero first, swop operands over - need this for others too, but must swop operator
         p_opsc(last,1,0);                        // operand 1 of compare

         if (curinst->opcix == 68 || curinst->opcix == 69) pstr(" %s ",t); // same op for JNE and JEQ
         else
       ...
       {    
         p_opsc(last,2,0);                        // operand 2 of compare
         pstr(" %s ",t); 
         p_opsc(last,1,0);                        // operand 1 of compare
       } 
     } 
   if (lwop < 0)
     { 
      // r0 is the wop     style  (R2+R1) == 0
      // assume last must be set....TEMP FIXUP
  ...
  //  t += strlen(t) - 1;         // allow for increment at end of for loop
    pstr(") ");
     ...
     switch (opcsub)
      {
       ...
       // must be an immediate LDW to be considered as an address.
       if (curinst->opcix == LDWIX)  d = add_ddt(o->addr, o->ssize);
       ...
       case 2:     // indirect, optionally with increment - add data cmd if valid
         ...
         o->bkt = 1;
         curops[0] = *o;                           // save curops[1] first

         if (anlpass < ANLPRT)
          { // get actual indirect values for emulation
       ...
       case 3:                                          // indexed, op0 is a fixed SIGNED offset
        ...
        if (anlpass < ANLPRT)
          { // get actual values for emulation, save [1] for push.
           if (curinst->numops < 3)  curops[3] = *o;     //curops[1];   // copy register details first
           opcsub = o->val;
       ...
       default:      // direct, all ops are registers ([1] already calc'ed above
         break;
      }

     //calc rest of ops - always register 1 already done
     for (i = 2; i <= curinst->numops; i++) op_val (g_byte(xofst + i), i);

     s->nextaddr = xofst + curinst->numops + 1; 
  }

INST *find_last_cmp(int min, int reg)
 {
   // finds last cmp for a register. 
    int cnt;
    INST *x;
    const OPC *opl;
...
void fpsh(OPS *o)
{
 // do a stack with fsptr, but loop around if it overflows (as safety)

fstack[fsptr] = curinst->ofst;     // full address
fstack[fsptr] |= o->addr << 20;       // register
fsptr++;                              // points to NEXT free
...
{
 // do a 'fake' stack 
 // really, we but loop around if it overflows (as safety)
 // temp lashup....
int ans, reg;

fsptr--;      // points to NEXT free
reg = fstack[fsptr] >> 20;       // register
ans = fstack[fsptr] & 0xfffff;   // value

if (fsptr < 0) fsptr = 31;       // loop
...
// int x;
  ...
  // check for single vect or vect list
  // immediate is normally a direct call if valid code addr
  // if indexed or indirect, flag count and wait for RET.
 
   // will have to add more for future auto arg detection 
   //(where pops ocurr first)
 
  o = curinst->opr + 1;

 // if (o->rgf)
 //  { 
 //   if (o->val > 0 && s->sub)   MUST check reg was POPPED first. 
  
  if (o->imd)
    {   // immediate value - add scan now if valid address.
      if  (val_code_addr(o->addr))  add_scan(o->addr, S_CSUB, 0);
    }
  ...
  if (curinst->inx)
     {
      ...
      // ops[0] is probably base
      // ops[3] (copied from ops[1])  is register.
      // go back to see if we can find a size
      // if no cmp found, assume calc'ed start in [1]
      // BUT CAN GET a single indexed item (A9L injection table etc)
      // only look for list if [0] is a valid address, otherwise if R is valid with small offset it's a SINGLE
      
      if (val_code_addr(curops->addr))
         {      // could be a list from fixed valid addr
          size = 0;
          y = find_last_cmp(s->start, curops[3].addr); 
          if (y)
           { 
            if (y->imd) size = y->opr[1].val; 
            if (size) do_vect_list(curops->addr, curops->addr+size-1);
           }
          if (!size ) check_vect_list(curops[1].addr, 0, s->curaddr);     //allow for non zero reg for start
         }
      else 
        {  // single offset not valid, but register may be - single vect pointer
         if  (val_code_addr(curops[1].addr))  do_vect_list(curops[1].addr, curops[1].addr+1);   
        }
 //      s->stk=0;
     }

    if (curinst->inr)
     // indirect - check for list

     // probably need to add a size check here too.
     // [0]->val is base of list, o->addr is register
     // temp cheat with check_vect...
      {

       check_vect_list(o->addr, 0, s->curaddr);     //allow for no zero reg for start
   //    s->stk=0;
      }
 ...
 // if (curinst->inx || curinst->inr)  s->stk++;
...
//   {
 ...
 /*  DO POP and push the 'wrong' way around. only pops with no push will be in the list ?
as candidates (and push zero) ....then the PUSH can look for the flag to check args.... 
and do NOT loop around.....

ADD STACK TO SCANPARS (say 4 levels) ??  MAY BE GOOD IDEA>>>>>>then all manipulation is LOCAL !!
...
x = find_ramlist(o->addr);
  if (x >= 0)
     { // found in ramlist - x is index
       if (ramlist[x] & 0x4000) DBGPRT("POPPED! ");
       ramlist[x] &= 0xBFFF;     // clear the flag
// how to tell value has changed ??
   //    if (ramlist[x] & 0x8000) val = g_word(ramlist[x] & 0x1fff);
   //    else val = g_byte(ramlist[x] & 0x1fff);
   //    if (o->val != val) DBGPRT(" %x -> %x ", val, o->val);
     }  */

 // if (x && o->val) 
    {                   // CHECK POP here for args...
  #ifdef XDBGX
 //  DBGPRT("FOUND from %x",x);   
   DBGPRT("PUSH R%x from (%x) ", o->addr, curinst->ofst);
   if (o->rgf) DBGPRT ("=%x", o->val);
   DBGPRT(" stk = %d", fsptr);
...
//   int x;
 ...
 //  SBK *c;
  ...
  //  wp = opctbl[curinst->opcix].wop;        // always 1 ...
    o = curops+1;

    if (!o->addr)  return;   // R0 throwaway

  //  fpsh(o);  // push this register onto internal stack
...
#ifdef XDBGX
   DBGPRT("POPW %x %d r%x = %x",s->curaddr,s->ctype, o->addr, o->val);
  if (s->sub) DBGPRT(" in sub %x", s->sub->start);
   DBGPRT(" stk = %d", fsptr);
  DBGPRTN(0);
#endif
}
...
void inc(SBK *s)
 {
   ...
   if (anlpass < ANLPRT) curops[opctbl[curinst->opcix].wop].val++;
 }
    ...
    while (x) // && cnt < 16)
     {
      x = get_prev_instance(x);
      cnt++;
     // if (x && x->ofst < s->start) x = 0; // can't do this !! CARD......
      if (cnt > 16) x = 0;             // test max 16 opcodes

      if (!x) break;
      opl = opctbl + x->opcix;
      if (opl->sigix == 14 && (x->inx || x->inr))  break;         // pushw
     }
...

{
  ...
  { 
    #ifdef XDBGX
     DBGPRTN("Hold set s %x c %x e %x", s->start, s->curaddr, s->end);  
    #endif 
    return;
  }  
 ...
 // Ford book states opcodes can have rombank(bank)+fe as prefix (= 3 bytes)
  ...
  if (!indx)
     {
       ...
       if (anlpass < ANLPRT)
         {
          #ifdef XDBGX
          DBGPRTN("Invalid opcode (%x) at %x from %x", x, s->curaddr, s->retaddr);
          #endif
          end_scan (s, xofst+1);
         }
       else  
  ...
  {  // bank opcode
    if (bkmap[tbk].bok)
     ...
     {
       if (anlpass < ANLPRT) DBGPRTN("ROMBNK - Invalid bank %d (%x)", tbk, curinst->ofst);
     }
    #endif
  }
  ...
  if (PSSRC)
    {
      if (opl->sce)
       {               // skip if no pseudo source
        ...
        fix_sym_names(&tx);            // for rbases, 3 ops, names, indexed fixes etc.
        shift_comment();              // add comment to shifts with mutiply/divide  - if (curinst->opcix < 10 && ops->imd)
        bitwise_replace(&tx);         // replace bit masks with flag names - ANDs and ORs  if (ix >  9 && ix < 14) ||  (ix > 29 && ix < 34)

    // invert jump logic if jump found
        if ((j & 1) && *tx) 
         {
           if (inx > 53 && inx < 72)                 // all cond jumps except djnz
           {
             ainx = inx ^ 1;        //if (inx & 1) ainx = inx-1; else ainx = inx+1;  // swop over if jump found.
             tx = opctbl[ainx].sce;
           }
           else j = 0;
        }
...
//           case 4 5 6 8 not used 
       ...
       case 7:
            ...
            pstr ("if (");
            if (curinst->opr[3].ssize)
               {       // add "signed" comment
                cmntinst = 0;
                aucmtofst = s.curaddr;
                sprintf(autocmt, "## (Signed)");
               } 
            if (inx == 56 || inx == 57) carry_src(&s, &tx, ainx);   // JC and JNC
            else  do_cond_ops(&tx, ainx);
            break;
     ...
     {          
      if (!PSSRC && (inx == 73 || inx == 74 || inx == 82 || inx == 83))          // CALL opcodes
       ...
       // add semicolon if no open or close backets
     if (*opl->sce && !j) pchar(';');  

     // add any reqd close bracket(s)
     find_tjump (s.nextaddr);
...
//           if (!i) pstr (" T%x", curinst->tval);          //new temp value
  ...
  auto_comment(ofst+cnt);                   // print auto comment if one created
  ...
  if (xsub)
   {
     ...
     xsub->nargs1 = c->size;       // and set sub size from cmd
   }
 ...
 void do_listing (void)
{
  ...
  *autocmt = 0;
  ...
  fprintf (fldata.fl[1],"\n# See '%s%s' for warnings, results and other info", fldata.bare, fd[2].suffix);
...
int check_sfill(SBK *s)
{
 ...
 #ifdef XDBGX
   DBGPRTN("SCAN %05x from %05x INVALID by repeated opcode",s->start, s->curaddr);
 #endif   
return 1;
}
...
void scan_blk(SBK *blk)
 {
     ...
     // need to check here that it's not a faulty filler block......
 if (check_sfill(blk)) {blk->scanned = 1; blk->inv = 1; return;}   
     
 put_scanpars(blk);                       // restore saved values

 while (!blk->held && !blk->scanned )
  {
...
DDT* find_next_ddt_struct(int ix, int ofst)
{ // 3.08
...
// probably need a maxend safety here..... and a sign check....
  ...
  if (n && ix < datch.num && ofst > n->start) rows--;
...
int set_startval(int size, int *sval, int *nval)
{
    // for funcs - to check start value
 ...
 */
  ...
  n = find_next_ddt_struct(ix,0);    // NEW - 
...
if (get_opstart(d->start))
{ return;         // scanned as code
  ...
  for (i = 0; i < BMAX; i++)
    {
     ...
     bank = i << 16;
     addr = b->minpc;               // PCORG; allows for missing bytes.....
     addr |= bank;
    ...
    // set_cmd (addr+0xa , addr+0xe, C_WORD,1);    // PCORG+0xa, cal pars and cmd
     ...
     set_cmd(0x200c|bank, 0x200f|bank, C_WORD);           // from Ford handbook
     
     set_cmd (0x2010|bank, j, C_VECT|C_CMD);    // interrupt vecs, by cmd
...
void check_bank_intv (SIG *s, int addr, int ox)
 {
    ...
    s->ofst += taddr;                   // fiddle to stop overlaps
    s->xend += taddr;
//          if (s->v[3])
...
int scan_start (int ix, int mapaddr)
{
 ...
 // mapaddr is fed in from last bank + 0xe0000 (+0x2000) 
    ...
    BANK *b;    
    SIG* s;
    int max, addr, taddr, bank, val, pass, x;    //, cnt;
       ...
       for (x = -2; x < 3;  x++)
        { 
         ...
         if (x < 0) 
            ...
            {
             s = find_sig(&hdr,0x2000|bank);          // hdr is pattern ref - but this is just a jump, really.... 
            }
  ...
  while (faddr < fldata.fillen && i < 6)           // 4 banks max for now
    {
     ...
     if (b->btype > 0)
      {                                        // found !! 8061 memap indicates 0xc000 (+a000), but 8065 to ffff
...
//  fldata.error = 0;
  ...
  x = fldata.fillen/0x10000;

  if (x < 1) x = 1;
  
  wnprtn("File appears to be %d banks",x);
...
short dissas (char *fstr)
{
 ...
 // ----------------------------------------------------------------------
  ...
  do_ddt();        // code, data, subnames. AFTER code  (need to add any scans from pushes here too....)
