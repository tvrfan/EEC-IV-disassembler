...
int do_jumpsig ( int inx, int foff)
  {               // calc jump address for the different jump types
   ...
   if (inx > 53 && inx < 84) 
     {                                           // all jumps)
      if (inx > 69 && inx < 73)  joff = negval(s_byte(foff + 2), 0x80) + 3;   // JB, JNB, DJNZ

      else if (inx == 80)  joff = 2;                   // skip - treat as  jump + 2
      else if (inx > 79 && inx < 84)                  //  short jumps and calls
...
int check_sign(int foff,int sgstate)
{
  ...
  ans = 0;
  val = s_byte(foff-1);          // get previous value
 ...
 while (ans)       //  need loop for repeat and optional patterns...
   {
    ...
    if ((sg & OPJMP)) sgv[tval] = do_jumpsig (inx,foff);      // jump or call - save dest, and can be sgv[0] 
    ...
    if (tval && !sgv[tval] && (sg & OPSPV))
      {
       if (inx > 69 && inx < 72)                   //jb or jnb 
        {
          sgv[tval] = (1 << (opc & 0x7));         // keep bit mask
          if (inx & 1)  sgv[tval] |= 0x1000;      // and state  JB=1, JNB=0
        }
        
       if (inx == 56 || inx == 57 || inx == 101 || inx == 102)
          { if (inx & 1)  sgv[tval] |= 0x10000;    // save carry state for stc,clc,jc,jnc
		  }
      }
