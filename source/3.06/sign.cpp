...
0x880b3017,  0x300f6,  0x04,             // *jnb   B1,Rf6,x keep bit - Signed IN ?  **JB** was opt
0x880b2013,  0x000a,                      // *jc    x keep carry state goto unsigned was opt
...
//PAT xstb = {subtb2,  "tscle", NC(subtb2),  0, 1, 0, nldf,   sdumx };    // scaled table foget this for now.
...
PAT avcl = {avct,   "AVCT",  NC(avct),   0, 0, 0, nldf,  sdumx  };    // altstack vector list
PAT timr = {times,  "TIMER", NC(times),  0, 0, 0, ptimep, sdumx  };    // timer list
PAT fnlp = {fnlu,   "FnLU",  NC(fnlu),   1, 0, 0, nldf,  fnplu  };    // func lookup - byte or word
PAT tblp = {tbl2,   "TBL2",  NC(tbl2),   1, 0, 0, nldf,  tbplu  };    // table lookup, later multibank
PAT tblx = {tblu,   "TBLU",  NC(tblu),   1, 0, 0, nldf,  tbplu  };    // table lookup, early
       ...
       if (opl->opt & T)
        {
          ...
          if (inx & 1)  sgv[tval] |= 0x1000;      // set if JB, clear if JNB
        }
        
       if (opl->opt & Y) sgv[tval] = inx & 1;                    // stc,clc,jc,jnc - save carry state NOT RIGHT !!
      }
...
// sx now moved along by each subr called
  ...
  if (ans > 0 && foff > ofst)                   // must have at least ONE matched param !!
    {
      // add sign to sig chain  make this an ADD_SIG again for manual add...
